...
describe("catalog ↔ registry consistency", () => {
  ...
  // [[C38]] sinkRunButtonOnly's persistence half: an external-effect arm flag must never
  ...
  it("no catalog class persists an `enabled` arm flag, and none constructs armed ([[C38]] sinkRunButtonOnly)", () => {
  ...
  // [[C28]] literalsIffEditable, the ONLY-IF direction. Declaring `literals` / `stringLiterals` is
  ...
  it("no class declares a literal map its component never edits ([[C28]] literalsIffEditable only-if)", () => {
    const offenders: string[] = [];
    ...
    expect(
      offenders,
      `These classes declare a literal map their component never edits ([[C28]] literalsIffEditable): ` +
