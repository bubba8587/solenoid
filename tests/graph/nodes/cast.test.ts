...
describe("Cast node", () => {
  ...
  it("casts a list to logical with a per-cell #VALUE! for an unparseable cell", () => {
    const r = cast("logical", ["true", "false", "maybe", 2]) as Array<boolean | import("../../../src/graph/errorValue").SolError>;
  ...
  it("returns #VALUE! when a scalar can't be cast", () => {
    ...
    const r = cast("number", [1, "2", "x", null]) as Array<number | null | import("../../../src/graph/errorValue").SolError>;
    expect(r[0]).toBe(1);
    expect(r[1]).toBe(2);
    expect(isSolError(r[2]) && (r[2] as import("../../../src/graph/errorValue").SolError).code).toBe("#VALUE!");
    expect(r[3]).toBeNull();
