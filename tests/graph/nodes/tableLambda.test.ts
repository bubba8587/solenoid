...
import { MapTableNode, ByAxisNode, ReduceLambdaNode, ScanLambdaNode } from "../../../src/graph/nodes/tableLambda";
import { LambdaNode, isLambdaValue, formatLambda, formatLambdaSig, undeclaredConsumerVars, type LambdaValue } from "../../../src/graph/nodes/lambda";
import { isSolError, type SolError } from "../../../src/graph/errorValue";
import { fromUnit, isUnitCell, magnitudeOf, type UnitCell } from "../../../src/graph/unitValue";
...
describe("Lambda wired into consumers", () => {
  ...
  // Every consumer now binds a wired lambda by NAME ([[C50]] lambdaBindsByName): its params must be the
...
describe("ScanLambda (SCAN)", () => {
  ...
  // A wired lambda binds to (acc, value, step) BY NAME ([[C50]] lambdaBindsByName): declaration ORDER is
