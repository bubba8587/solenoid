...
describe("text form: unit cases", () => {
  ...
  // Every drawn-cable field has to survive the JSON save, and the JSON save is
  // GENERATED by round-tripping through this text form — so a field the sidecar drops
  // is a field that silently does not save.
  it("round-trips every drawn-cable field, per-point angles included", () => {
    const g: SavedGraph = {
      v: 2,
      nodes: [],
      connections: [],
    ...
    expect(reloaded.drawnCables).toEqual(g.drawnCables);
    // And it is a FIXED POINT: a second pass changes nothing.
    expect(writeTextForm(reloaded)).toBe(writeTextForm(g));
  });

  it("omits the drawn-cable sidecar entirely when there are none", () => {
    const text = writeTextForm({ v: 2, nodes: [], connections: [] });
    expect(text.includes("drawnCables")).toBe(false);
