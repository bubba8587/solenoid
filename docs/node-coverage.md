...
- **Genuinely type-agnostic**: the wildcard ladder (`../tree/specs/values/socket-lattice.md` § The wildcard ladder keeps rank). `any` (a gray circle, one value of any family), `anycombo`, `anylist` and `anytable` (its 1-D and 2-D siblings), `anydata` (anything up to a matrix) and **`trueany`** (a hollow gray circle), which accepts anything.
...
- **Rank & Percentile**: one order-statistics node with LARGE / SMALL, RANK.EQ / AVG and PERCENTILE / QUARTILE / PERCENTRANK, including the INC and EXC forms, as ops. Each takes a list and one scalar (a k, a value, a p or a quartile) and gives a number; PERCENTILE and PERCENTRANK are each other's inverse. RANK and PERCENTRANK share the Value key. Missing cells are skipped and errors propagate. An out-of-range quartile is `#DOMAIN!`, on the card as in the formula.
...
- **Visual nodes** (`nodes/visual.ts`, the `display` kind, drawn with recharts unless noted). Each figure node is terminal: it emits a chart value on the `chart` socket and never passes its input through, so each one embeds in a Report like a chart. The figure mechanics are `../tree/specs/computation/chart-figures.md`.
  ...
  - **Chart**: 12 kinds (column, bar, line, area, scatter, XY line, pie, radar, radial bar, funnel, composed, bubble). Scatter, XY Line and Bubble read columns by name from the options ([[D87]] xyColumnMapping).
  - **Merge Plots**: overlays several x/y-plane charts on one plot, each keeping its own mark and styling; with a scatter, XY line or bubble among them the plot is one numeric x/y plane. A polar or payload figure is refused with `#TYPE!`.
...
- **Currency** (Frankfurter ECB FX): a **Spot / History** `mode` swaps the sockets in place, pruning the departing cables first (`../tree/specs/canvas/input-cable-pruning.md` § The ordering rule).
...
- **Properties target.** A `cube` of rows (a Frame widens) goes to notes' YAML frontmatter, keyed by a `path` column. There is no separate `WritePropertiesNode`.
  ...
  - **One YAML writer.** The pure `frontmatterPatch.ts` is the one writer of a note's YAML ([[C101]] onePatchPath). `patchFrontmatter` edits line-level over the raw text:
    ...
    - The keys written are the `keys` CSV when given, else every column except `path` and the read-only file built-ins (`BUILTIN_READONLY`: path, name, folder, ext, size, created, modified, links, embeds, date), which describe the file rather than the note. A row with no `path` cell is skipped. `tags` is Vault Folder's `file.tags`, which folds in the body's inline tags, so a written `tags` leaves out any tag only the body holds (`frontmatterTags`) and a round trip never copies inline tags into the frontmatter.
...
- The provider switch prunes departing cables through `dropInputCables` + `dropStrandedFrontmatterCables` (`../tree/specs/canvas/input-cable-pruning.md` § The ordering rule).
...
- A pack file imports only `packShared`, its `<id>Formulas.ts`, `rete-nodes` and type-only app seams, never core internals. `<id>Formulas.ts` holds the pack's `formulas` and imports only rete-free kernels (`../tree/specs/floors/engineering.md` § The formula path is rete-free).
...
- **Electricity & Circuits**: 23 presets (SI units) + Electrical FC units and an SI-prefix format (3 significant figures, 4700 is "4.7k"). Ohm's Law and dBm ↔ Watts are locked Equation presets (solve any way, or truth-check). Custom nodes:
  ...
  - **AWG Wire**: diameter (mm), area (mm²) and copper resistance (Ω/km). Ampacity comes from the NEC 310.16 copper 75 °C column, which rates the integer gauges 14 through 4/0; any other gauge's ampacity is blank. Outside 4/0 (−3) to 40 is `#DOMAIN!` on every output; fractional gauges are allowed.
