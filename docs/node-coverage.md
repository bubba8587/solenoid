...
- **Visual nodes** (`nodes/visual.ts`, the `display` kind, drawn with recharts unless noted). They all flow the `chart` socket, so each one embeds in a Report like a chart:
  ...
  - **Record**: Frame rows as labeled boxes on a text-defined grid, with four views on one op selector: **Card** (one row, picked by the pager or a wired 1-based `Row`), **Gallery**, **Board** (lanes keyed by a `Group by` column) and **List**. The card itself never draws the grid; the drawn figure appears wherever the chart output goes. Seed: `record-cards`. The layout grammar, caps and rendering are `../tree/specs/computation/chart-figures.md`.
...
- **Chart UI and options.** Twelve figures take an `options` string (`key=value;…`, matplotlib's names, [[C96]] chartOptionsAreMatplotlib), typed inline or built by the **Chart Builder** node. The keys, which figure reads which, the Chart card's pickers and the render paths are `../tree/specs/computation/chart-figures.md`.
...
- It runs in the sandbox Worker under a 1 s wall clock (`../tree/specs/computation/script-sandbox.md`).
...
**Annotation** (the mechanics of Notes, Reports, Knap, mail merge, vault writes and export are `../tree/specs/documents/reports-and-notes.md`):
