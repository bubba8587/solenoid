...
`dragHandler.guards.down` patch). An expanded group's RF wrapper is pointer-transparent
(`sol-group-open` in `flowModel.nodeClassName` + flow.css, the Conduit pattern); the header,
14px `__band` divs along the dashed edge and the grip claim pointers, and the whole body re-arms
once the group is SELECTED (so a selected group drags by its face — new over the rete era, which
never armed the body). Interior presses fall through to the pane: pan/lasso work inside a group,
and an interior right-click now opens the CANVAS menu, not the group's (behavior change, deliberate).
Collapsed groups stay fully draggable. Doc: subsystem-invariants § React Flow surface contract.
**Script editor parity with Expression** (author ask: highlighting not handrolled, popup, wide).
`jsSyntax.ts` maps lezer's real JS grammar (`@lezer/javascript` + `@lezer/highlight`, new deps)
onto the `.fx-tokens` palette (+`fx-kw`/`fx-comment`); `JsEditor.tsx` is the FormulaEditor
overlay technique (transparent textarea over a highlighted `<pre>`, `white-space: pre`,
scroll-synced) shared by the card field and the new `ScriptPopup` (PopupShell chrome,
`scriptPopupStore`, commit ONCE on exit like the formula popup, Ctrl+Enter applies). The card
field opens at the source's own height (64–360px), then the grip owns it; grip + expand button
seat inside the editor box corner (ScriptNode.css overrides). Expression/Script/Equation joined
the WIDE card tier (`nodeWide`): a typed line wants column width.
**Script Recalculate button**: `scriptIsVolatile` (scriptRun.ts, source regex for
Math.random / Date.now / argless `new Date` / crypto.randomUUID·getRandomValues /
performance.now) shows the shared RecalcButton (F9 semantics). Pinned in script.test.ts.
**Seed `script-tour` rebuilt** ("Script: worked examples") — the 123-node per-socket tour is gone;
four substantial scripts, one result shape each: Monte Carlo π (seeded LCG → number), Collatz →
Sparkline + chained text summary, 30-year amortization (rows, header + numbers under Auto),
Friday-the-13ths (returned `Date`s under the Date toggle). No groups. Values verified live via
`scripts/script-seed-probe.mjs` (loads the seed on the dev server, dumps readouts, measures the
...
**Seed `script-tour`** shipped as an exhaustive 123-node per-socket tour; the author replaced it
the next session with four worked examples (see 2026-08-28b).
**Found by the seed, fixed:** `retypeOutputCables` judged a cable against an ADOPTIVE input's
adopted type, so a Script/Expression whose result reconciled from rank 1 to rank 2 pruned its own
cable into a Display that had adopted the rank-1 type (two cables vanished on load). It now judges
by the socket's declared `base` (adoptKeepsCables); pinned in `fcReconcile.test.ts`.
**Console hooks restored:** `seedTune.ts` / `census.ts` lost their importer in Cutover A
(99d3a272), so `scripts/tune-seeds.mjs` and `card-css-census.mjs` timed out waiting for
`window.__solenoidTuneSeed`; `main.tsx` now dynamic-imports both in DEV.
...
- **D5: Select Columns + Drop Columns → one keep/drop node (Jeff/A5, 2026-08-25).** One `ColumnsNode` (class
  name kept, internal) with a Head-style `keep | drop` op selector; same sockets, the column-row label follows
  the op. JS-only (the `select`/`drop` lazy verbs and their Polars mirrors were untouched). Old
  SelectColumns/DropColumns saves load as Placeholders. Op fx names KEEPCOLS/DROPCOLS declared to dodge the
  list-drop `DROP` uniqueNameMap clash. **Renamed away from a bare "Columns" (author 2026-08-25, NAME-2):** it
  would read as Excel's `COLUMNS()` count. The card is now named by its op — "Keep Columns" / "Drop Columns",
  label follows the selector — and the Add menu shows two bare leaves (host `keep-columns` + `leafOps` so drop
  gets its own leaf, no colon row). SELECTCOLUMNS / KEEPCOLUMNS / DROPCOLUMNS all #TYPE!-refuse on the formula
  surface, redirecting to the new labels. New rule + pin: `frameSurfaceNames.test.ts` denylists bare
