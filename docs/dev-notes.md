...
### SESSION DIGEST (2026-09-09 — Personal Finance seed trimmed, 171→163 nodes)
...
green. **Not done (flagged for the author):** two orphan nodes with no consumer — `Inflation %/yr`
(NumberInput) and `Sum by category` (GroupByFrame); left in pending a call on whether they're
intentional stubs.
