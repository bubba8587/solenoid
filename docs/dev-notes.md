...
**Seed `script-tour` ("Script: every surface", Values & units, order 215; 123 nodes).** Two groups: In,
one Script per socket family `anydata` can read (number/list/numlist, text/strlist/strcombo,
date/datelist/datecombo, complexcombo/complexlist, logical/logicalcombo/logicallist, the four
Table Input element types, anytable/anylist/anycombo/any/trueany, plus a Script → Script chain);
Out, one Script per result type × rank (Number/Text/Date/Auto × scalar/list/matrix) into a typed
consumer (Arithmetic, Sparkline + a frame-row Table Size, Transpose, PROPER, Regex, DateDiff, DAY,
Cast, List Sort, Expression, Chart, 7-Segment, NOT, and a blank + `#DOMAIN!` cell list), and a
Report addressing two Scripts by name. Every result was dumped from the live page and the Display
labels match. Generated by a scratch script; group geometry fitted by the tuner.
