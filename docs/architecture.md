...
| `selectionOps.ts` + `components/SelectionActionsBar.tsx`/`selectionActions.css` | Align/distribute deltas (pure) + the bottom-centre overlay pill (≥2 nodes selected) that surfaces them outside the Command Palette |
