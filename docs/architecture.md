...
| `zoomAt.ts` | Frame a node set over a structural `ZoomSurface`: RF's `getNodesBounds` + `getViewportForBounds` (padding 0.1, never zooms IN past 1), zoom floored to the snap step |
...
| `flow/flowModel.ts` (+`.test.ts`) | The headless graph model builder (real NodeEditor + DataflowEngine + coercion + guards, no view) + the RF projections `toFlowNodes`/`toFlowEdges` (plain RF-shaped objects, no @xyflow import — node-vitest-testable): a group member projects as an RF child (`parentId`, position relative to the group, parents first), `toFlowPosition`/`fromFlowPosition` convert at the boundary, `nodeZIndex` (groups −2 < conduits −1 < nodes 0) |
