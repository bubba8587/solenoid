...
- Lando — B1 follow-through: the Records seed exhibits + Function Reference / Inspector copy. B1 (329d613f, b578a206) merged. E2 LANDED (8c9c6f11), merged. FC inherit pick + F5 findings LANDED, merged.
- Han — lead; merges; walking the 1.4 cut with the author one item per turn (Track A, B1, B2 settled; at B3).

**Awaiting the author's eyeball:** three finance cards (Finance > Other / Bonds), the FC `—` inherit pick, the Display corner expand on frames/tables/lists, the socket hover peek (A3), the mobile delete button inside a drill-in (E2), the Record List view / cardsize / #title / clamp (B1); from earlier: the anydata hollow-square glyph + Socket Legend row; the catalog description sweep (8d5c8f74); Merge Plots figure + expand button; Vite 8 (desktop `tauri build` ride-along result per A3's last message).
