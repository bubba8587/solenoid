...
6. **Deferrals reconcile (doc-only, the standing "reconcile, don't append" rule):** (IN PROGRESS) walk `docs/deferrals.md` "Pushed to 1.4 / 2.0" + "Only if the trigger returns" and verify each entry against CODE and today's landed work (Merge Plots, lazy handles, Vite 8, Slicer, the catalog sweep). Delete entries that landed or that `backlog.md` now owns (one home per fact); do NOT ratify/amend `out-of-scope.md` (author-present). One commit listing what was removed and why.
