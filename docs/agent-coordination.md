...
2. **ACCRINT + ACCRINTM → ONE card** (same parked list; author may veto). `finance.ts` `AccrintNode` + `AccrintMNode` (components `AccrintNode.tsx` / `AccrintMNode.tsx`): one `AccruedInterestNode`, op Periodic (ACCRINT: issue/first_interest/settlement/rate/par/frequency/basis[/calc_method]) vs At maturity (ACCRINTM: issue/settlement/rate/par/basis); spec-table the per-op inputs, `fx` = the Excel name, carry issue/settlement/rate/par/basis across the switch. `financeInvariants` + parity + nodeOps / formulaNodeCoverage / seeds / catalogRegistry / uiCopy.
3. **BondPrice + OddCoupon → ONE card** (same list). PRICE/YIELD family + ODDFPRICE/ODDFYIELD/ODDLPRICE/ODDLYIELD as ops on one bond card; the odd variants add their extra date params per op (first_coupon / last_interest) via the spec table. Keep the PRICE↔YIELD and ODD* round-trips in `financeInvariants.test.ts` green.
