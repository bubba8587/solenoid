...
7. **Rust suite** (mechanical): `cargo test` in `src-tauri/` (cargo 1.94 is installed). Hygiene C edited `engine/tests.rs` and the day's frame work must not have moved the parity corpus. Report pass/fail counts; if red, stop and report the failing test names, don't fix.
