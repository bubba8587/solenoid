...
### Agent 2 (solenoid-ca) — CLAIMED: Per-card CSS census (item 2 below), after Merge Plots docs reconciliation. Staged QUEUE:
...
4. **Vite 8 upgrade** — follow the `docs/plans/vite-8-upgrade` plan exactly (the persisted-type gate is the point; stop and report if class names mangle or the license file goes empty).
