...
The `format` annotation is stamped on a node's FrameValue outputs by the input-coercion wrapper from `frameFormatStore`, per node and column; a nearer pick overrides a farther one. Unit and format behavior per verb is listed under each verb; unit rules themselves belong to `unit-flow.md`.

### Frames and Cubes

A Cube never enters a Frame socket ([[E2]] cubeNeverNarrowsToFrame). A verb card that accepts a Cube declares a cube-adoptive input with `noWidenInputs` and either runs its own cube branch (the cube row verbs under Eager verbs outside the seam) or calls `flatCubeToFrame`, which types each column through `typedColumn` with the cube column's carried type, reads a `UnitCell` as its bare base-SI value (the unit is not recovered), and answers `#SHAPE!` (`Column "<name>" holds nested cells; this reads flat rows`) when any cell is a list, Frame or Cube. `frameToCube` goes the other way: depth 1, element type and format carried, locked cells tagged.
