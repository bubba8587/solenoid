...
| `invalidate(editor, engine, changedId?)` | No `changedId`: `engine.reset()`, returns `null`. With one: computes the cone and deletes each member's cache entry by hand, returns the cone. |
...
| `fetchAll(editor, engine, onNode?)` | Fetches every node in `editor.getNodes()` order, collecting a map of id to outputs. A fetch that rejects with `Cancelled` records `null`; any other rejection propagates. |
...
`resolveTrigModes(editor)` runs before every fetch because a trigonometric Math node in automatic angle mode reads its resolved mode during `data()` (`unit-flow.md`, the one compute-time unit read).

The headless runner (`scripts/run-graph.ts`) calls `computeAll`. The app's pass (`processGraph`) and a composite's private engine call `loopMembers`, `downstreamClosure` and `seedLoopErrors` from this file and run their own invalidation and fetch loops, described below.
...
Error sinks (`registerErrorSink`; the Problems store registers one) receive at most one report per node per call: the first error found on the node's own outputs by `findCellError`, a bounded scan of the head and a stride sample of each container (`error-values.md`, "Per-cell errors surface through a bounded scan"), or `null` when the outputs are clean, which re-arms the sink's edge detection. The Problems store logs only at an error's origin node, and not while a rebuild scope is open.
...
Coercion turns each arriving value into the shape of the rung the consuming socket declares, so `data()` is written against one shape per input. Acceptance (which cables may exist) is socket-driven and lives in `socket-lattice.md`; coercion is the runtime half and never crosses element families except by the one bridge ([[D11]] noAutoCross).
...
An unparseable field is `null` in place, never dropped, so later positions do not shift. Literal maps are restored on load only onto classes that declare them (`inline-literal-maps.md`); `coerceInputs.test.ts` fails when a catalog node with a typeable list input does not declare `stringLiterals`.
