...
- **Stacking.** [[C65]] places the bars at −3 in the stacking ladder, below groups (−2), Conduits (−1) and ordinary nodes (0). No code sets that value. `nodeZIndex` in `flowModel.ts` stamps only the nodes (groups −2, Conduits −1, the rest 0), and the standoff SVG carries no z-index, so the bars' place is simply where React Flow puts the viewport portal: after the edge and node layers.
