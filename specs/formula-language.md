...
The per-name reasons Solenoid overrides a Formula.js function are `specs/formulajs-divergences.md`; this spec does not repeat them. Computed-column name resolution is `specs/computed-columns.md`. The Equation node's symbolic rearrangement of the same AST is `specs/equation-solver.md`. The dimension pass that runs beside the numeric evaluator is `specs/unit-flow.md`.
...
3. When any input carries a non-dimensionless unit, `dimEval` computes the result's dimension from the AST, the input dimensions and their currency codes. An error there is the result; a known dimension tags each numeric cell; an indeterminate one drops the unit (`specs/unit-flow.md`).
