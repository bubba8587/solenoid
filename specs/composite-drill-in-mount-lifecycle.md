...
5. swaps the chrome's command slots to this level: selection, Tidy and Cleanup, the touch delete button, and docked-FC repositioning (`specs/react-flow-surface-contract.md`);
...
Anything the drill-in shares with the main canvas comes from the same module, never a copy: `installFlowPinch`, `installTouchCardPan`, `installWheelZoom`, `FlowCableEdge`, `SolNodeAdapter`, `minimapFillForNode`, and the Tidy factory `makeArrangeFn` with `makeCleanupFn` (`specs/pointer-gestures.md`). The drill-in's Tidy refits the view afterwards, except for a Tidy scoped to one group.
