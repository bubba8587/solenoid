...
Serves [[C10]] socketLattice: type separation and dimensional flow, derived from the family × rank product. Filled from the tree and the former `subsystem-invariants.md` § Socket lattice; built by an agent that never reads the tree. `docs/socket-reference.md` is the per-variant reader's reference and stays; this file is what a builder implements.

## Purpose

Element families never auto-cross (Cast required; sole bridge `logical↔number`); values flow freely up in rank (scalar→list→matrix→frame→cube). Legality is DERIVED from the family × rank product, never enumerated per pair.
...
3. **The bridge is built by the same predicate.** `logical ↔ number` is the ONE cross-family bridge (0/1 ⟷ TRUE/FALSE, the multiply-by-a-condition idiom); it is dim-mirrored through `dimFlows` on both sides so the combo→scalar exception cannot drift out of one side (the 2026-07-25 gap: `logicalcombo → number` blocked while `logical → number` flowed). `coerceInputs` does the runtime conversion. [[D11]] noAutoCross.
...
5. **Cross-type dimensional edges are explicit in `accepts()`** because rank derivation cannot express them, and they are the edges every rework lost. The complete list:
...
8. **Adoption keeps rank on the rank-bearing wildcards.** When a cable lands on an adoptive input, `adoptTypeForBase`: an `anylist` / `anytable` base keeps its rank and adopts only the wired ELEMENT family (a wider wire is taken verbatim, a narrower one is lifted to the base's rank in the wired family); a family-less wire carries nothing to adopt, so the port keeps its base, and the rank-0 / combo / `anydata` bases follow the same family-less rule; `trueany` adopts verbatim. `projectTypeToBase` is the OUTPUT-side mirror (WRAPROWS widens, flatten narrows) and its family-less branch must agree with the input side. [[D15]] wildcardsKeepRank, [[E3]] adoptKeepsCables.
9. **Adoption never touches connections and is never saved.** `settleWildcardTypes` runs the adoption pass to a fixpoint (a chain of N passthroughs settles in ≤ N passes; the cap guards a `#CIRC!` loop), alternating with Conduit lane typing to a JOINT fixpoint because each can feed the other. One frame-shape walk serves the whole fixpoint: shape depends only on topology and literal config, which no pass mutates. Output policy comes from the node's `passthrough()` declaration, the ONE source unit flow also reads; branch votes: an unwired or error-only source abstains, a `trueany` vote vetoes the agreement; a rank-crossing reshape adopts the element family at its OWN declared rank. [[E3]] adoptKeepsCables, [[E7]] trueanyNeedsPassthrough.
10. **Two wildcard predicates, on purpose.** `isWildcardType` names the RANKLESS rungs only (a rank-bearing wildcard is a real dimensional constraint, not "untyped"); `isWildcardRung` names every family-less rung and is the one to use for FAMILY resolution. Every see-through-an-untyped-hop check calls `isWildcardType`. [[E4]] oneResolvePredicate.
...
A builder that finds this spec silent stops that part and runs `python tools/dte.py gap specs/socket-lattice.md --title "..." --by <name>`; it never improvises.
