<!-- [[C84]] tidyTranslatesOnly, [[D63]] lockedGroupIsObstacle, [[D64]] oneSizeRead, [[C89]] standoffsSolveLast -->
...
1. React Flow's own measure of the mounted card (`view.measured`), which costs no DOM read.
2. The live rendered size (`offsetWidth` / `offsetHeight`), once both are non-zero.
3. The node's stored `width` / `height`.
...
- The root options are `ELK_ROOT_OPTIONS`: `elk.algorithm = layered`, `elk.hierarchyHandling = INCLUDE_CHILDREN`, `elk.edgeRouting = POLYLINE`.
...
`tidyLayoutOptions` turns the settings into ELK options. Both call sites (the main canvas and the drill-in) read them at layout time through `tidyOptionsFromSettings`. The popover off the top-bar Tidy button (`TidyOptionsPopover.tsx`, `.solenoid-tidy-options`) mirrors the Settings rows.
...
These steps run in order after ELK returns:
...
`makeCleanupFn` runs these steps in order:

1. Clear the node and cable selection.
2. Tidy every unlocked group's members with `skipPush: true`.
3. Wait two frames so the deferred FC snap-backs land, then autofit every unlocked group.
