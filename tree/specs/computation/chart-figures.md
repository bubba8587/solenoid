...
---
<!-- [[C100]] chartIsAValue, [[C96]] chartOptionsAreMatplotlib, [[D75]] builderExposesEveryOption, [[B2]] webTryDesktopFull, [[C71]] noBarEditing, [[C63]] oneRecordNode, [[C94]] formatFamilyGates, [[C26]] opArgDistinct, [[C103]] untrustedContentSeams, [[D87]] xyColumnMapping -->

# Spec: Chart figures

Serves [[C100]] chartIsAValue, [[C96]] chartOptionsAreMatplotlib, [[D75]] builderExposesEveryOption, [[B2]] webTryDesktopFull, [[C71]] noBarEditing, [[C63]] oneRecordNode, [[C94]] formatFamilyGates (the Format Controller's `chart` family) and [[D87]] xyColumnMapping. It covers what the system does and blocks, and the decision each behavior serves. A WHY that isn't in a node belongs in one.
...
Scatter, XY Line and Bubble plot points on a numeric plane ([[D87]] xyColumnMapping). `buildXY(op, input, options)`:
...
- **x** from a number column is the cell; from any other type the cell's formatted text, and x is its index in `xcats` (distinct texts in first-seen order), so the axis reads as categories.
...
- **Series.** One per y column, or with `by`, one per y column per distinct formatted `by` value (a null cell groups as `—`), in first-seen order, each holding only its group's rows in row order. The name is the y column's, the group's value, or `y · group` when there are several y columns.
...
| `aspect` | `equal` or `auto` | auto | `equal` pads the axis ranges so a unit spans the same pixels on x and y (a circle draws round), with round-number ticks. |
...
| Scatter, Bubble | as Column, plus `x`, `y`, `s`, `c`, `annotate`, `by`, `linestyle`, `aspect`, `xmin`/`xmax`, `linewidth`, `markersize` |
| XY Line | as Scatter, plus `marker` |
...
- **XY.** One recharts `ScatterChart` on numeric axes (domain `auto` unless `xmin`/`xmax`/`ymin`/`ymax` pin a side, which then clips), each series split at its gaps into runs, each run one `Scatter` whose joint `line` draws the `linestyle` (dashes `6 4`, dots `1.5 3`, dash-dot `6 3 1.5 3`) in row order. With `xcats` the x axis is the category indices `[0, n-1]`, 8 pixels padding, every tick shown up to 12. A marker's radius is the series' `markersize`, else the option, else 3 on a lineless series and 2 on a lined one; with an `s` range it is the radius of an area from 40 to 420 square pixels, linear in `s` over the range (a missing size draws the smallest). Its fill is the height ramp at `c`'s place in the range for a numeric `c`, the palette color of its category for a text `c`, else the series color: the palette by index with two or more series, else `color`, else the default mark color. Opacity is `alpha`, else 0.55 on sized markers and 1 elsewhere. `annotate` text draws right of each point (`sanitizeChartLabel`, 12 code points). Under the plot, a `SeriesLegend` row with two or more series (click spotlights, line swatches when any series has a line), a category legend for a one-series text `c`, and a colorbar row for a numeric `c` (the column name, the range's ends and a ramp swatch). An axis with both bounds pinned by the options draws round-number ticks (`niceTicks`), about one per 60 pixels on x and 40 on y, never fewer than three. `aspect=equal` computes the plot box from the figure size, pads the shorter range and draws the same round-number ticks.
