...
---
<!-- [[C100]] chartIsAValue, [[C96]] chartOptionsAreMatplotlib, [[C97]] rechartsLazyChunk, [[C71]] noBarEditing, [[C63]] oneRecordNode, [[C94]] formatFamilyGates -->
...
- The table is flattened depth-first. The task column is the first of `task`, `name`, `title` (case-insensitive), else the first column holding any text; with none, `#VALUE!` "Gantt needs a Task column naming each task". A child column (`tasks`, `children`, `subtasks`, `steps`, or a column of nested tables that have a task column) recurses one level deeper. A row with a blank task name is skipped.
...
| `radarscale` | `axis` (`normalize`, `normalized`, `independent`) or `shared` (`raw`, `absolute`) | axis | Multi-series radar radius. |
...
A key a renderer does not read is inert on that figure. This is what the renderers read now:
...
The Chart Builder node has one input per field (24 text, 6 number) and one output, `result` (a string). Each field is the wired value, else the card literal; a wired blank means the field is unset, never the literal. Its `target` (one of the 22 `CHART_BUILDER_TARGETS`, default `column`; a stale saved target loads as `column`) only chooses which rows the card offers (`chartBuilderKeys`; the Gantt target offers fewer keys when `layout=calendar`). A row the target does not read stays visible and dimmed while it is wired or holds a value, and every set field serializes regardless of target, so one builder can feed several figures. Toggles store `on`/`off`; a select's default option stores `""`.
...
- **Tooltips.** Values format with `formatScalar`. The single-series tooltip shows `#n` (1-based) and the value; categorical tooltips show the value only; multi-series tooltips list each series with a swatch; a tooltip value that is an object shows its `code`.
...
- **Dial.** A half-ring arc for `value` clamped to `[0, 1]` over a track, with the percent (one decimal) centered and `0%`, `100%` at the ends. Its size is the given `size`, else the width clamped to 120 to 200 pixels, drawn 0.55 times as tall.

## The CSS and canvas figures

- **KPI.** The formatted value (`formatScalar`) with the unit suffix. When `prev` is a finite number, a delta line shows an arrow (up, down, or a bar for no change), the absolute change and, when `prev` is not zero, the percent change against `|prev|`. It is green when the direction is good (`goodUp` decides), the error color when bad, dim when unchanged.
...
- **Record.** Card: one CSS grid of boxes at the payload's placements; a title box drops its label; an image box draws the picture; an empty box shows its hint muted or a dash. When a surface passes `recordNav` and there are two or more records, a pager (previous, `index / total`, next) sits under the card and never clips. Gallery: a masonry of cards, tracks aimed at 130, 170 or 230 pixels for `s`, `m`, `l` (bands 110 to 190, 140 to 260, 190 to 340), 6 pixel gap, each card placed in the shortest column, tiles hidden until the first measurement settles. Board: one lane per `lanes` entry. List: per record, the title field's text, then `label: value` lines. `+N more` follows when `more` is set. The figure's height follows its content; the given height is ignored.
...
- **Calendar Heatmap.** Values on the same day sum. Weeks run Monday first; the grid ends at the last day's week and starts at the later of the first data day and a year before the end, limited to the weeks that fit at 3.2 pixels a cell. When data starts before the drawn grid, `last N wk` shows in the corner. Month initials label the top, `M`, `W`, `F` the rows. Every in-span day draws a sunken cell; a day with a value adds the accent at opacity `0.16 + 0.84 · t`, where `t` is the value's place between the drawn minimum and maximum (0.75 when they are equal).
...
- **Contour.** Each grid cell is shaded by bilinear interpolation in 6 by 6 subquads through the palette's height ramp; a cell with any missing corner stays blank. Iso-lines by marching squares at `levels` evenly spaced heights strictly inside the data range (at least 2), splitting saddles by the cell mean. Corner hints show x min, x max and y max.
...
- **Vector Field.** One arrow per cell, colored on the height ramp by magnitude relative to the largest, length `0.15 + 0.85` of that fraction times 46% of the cell; positive v points up. A cell with a null component draws a faint dot.

Tick labels on the canvas figures are compact: three significant figures with K, M, B above a thousand.

## Mermaid

`MermaidView` loads `mermaid` on first use and initializes it before every render with `securityLevel: "strict"`, `theme: "base"` and theme variables read from the live CSS variables (surface, text, border, accent, the pie palette). It validates with `mermaid.parse` first, then renders the SVG into its host element. A blank source renders nothing. A parse or render failure draws a `Diagram error` box with the message as its tooltip, never Mermaid's own error graphic. A theme change re-renders.
...
The popup (`ChartPopup`, one instance mounted in App, state in `chartPopupStore`) holds a snapshot of the value, not a subscription. It draws through `ChartFigure`, with `virtualize` on for Gantt (windowed rows), and offers Copy SVG for a Gantt (`ganttSvg` at the current width). For a Record card whose Row is unwired it pages with the on-figure pager and the left and right arrow keys, stepping the node's Row literal and swapping in the fresh value (`recordNav.ts`, which finds the Record node by walking up to 8 single-inlet hops). A Sparkline's popup is drawn from the series it was opened with.
...
The webpage export and Write to Obsidian take a chart as SVG from the source node: a registered serializer first (`registerChartSvgProvider`, keyed by node id), else the largest `<svg>` in the node's element that is not card chrome, serialized with computed styles inlined (`canvasCapture.ts`). The Gantt card and a Gantt drawn in a canvas Display register `ganttSvg(payload, { width: 1000 })`. Write to Obsidian rasterizes that SVG to a PNG asset. A figure that draws on a canvas, or a node not on the live canvas, exports nothing.
