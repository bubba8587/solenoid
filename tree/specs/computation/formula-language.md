...
**MUST:** every `registerInternal` name has an `EXCEL_IMPL_META` entry declaring its `returns` and `arity`, plus `rank` and `listArgs` where they apply. Routing, rank checks and list handling are derived from that entry and declared nowhere else ([[engineering#Lists of names are generated]]). A registration with no declared contract leaves its routing to be guessed, or kept by hand in a second list, which is the drift [[engineering#One declaration per fact]] exists to stop.
...
- **The check.** A catalog leaf label or a `NODE_OPS` op label written in all capitals (dotted ones such as `STDEV.S` included, each token of an "X / Y" enumeration counted on its own) must be in `formulaFunctionNames()` or be a `FRAME_SURFACE_NAMES` key; every other label is Title Case. The allowlist: the acronyms PCA, BMI, TDEE, SVG and KPI and the Solenoid-only op ISNULL stay all-caps though they don't dispatch, and the flagship `Convert` stays Title Case though `CONVERT` dispatches.

- **GROUPBY and PIVOTBY** are frame nodes. Excel runs them as formulas; Solenoid's formula surface refuses them through `FRAME_SURFACE_NAMES` (Frames don't flow through formulas, [[C15]] matricesInFormulas) and the refusal names the node, so the capitals still point at something the user can use. The [[D23]] test admits the map's keys. Frame verbs with no Excel function (Unpivot, Nest, Rename and the like) stay Title Case. If the stack merge lands, Append and Bind Columns become VSTACK and HSTACK. Reopen if the formula surface starts accepting Frames.
...
**MUST (containment):** a matrix reaches a function whole only when its registration declares `matrixArgs`, and a complex value only when it declares `cxArgs`; every other case takes step 9 or 10 above. So Formula.js only ever receives lists and scalars, permanently. The three complex exemptions are those that must see their operand as it is: the `NULL_INSPECTING` value-passers (IF hands a complex branch through, and the type checks must see it), the error handlers (a catcher sees its raw operand), and the whole-list natives, whose position-preserving ops treat elements as opaque (`REVERSE` of a complex list is legitimate).

Formula.js's array functions are written against 2-D spreadsheet ranges and have untested quirks, and one has been caught changing its arguments in place (CHISQ.TEST). When [[C15]] matricesInFormulas lifted the old formula rank cap, which had partly been containing that, the containment had to stay. Without the gate, UNIQUE, SORT, MODE.MULT and FREQUENCY dispatched through Formula.js and broadcast element-wise, so `UNIQUE([3,1,3,2])` answered a column of singletons, and TREND, GROWTH, LINEST and LOGEST returned plausible-looking garbage the same way. Complex numbers had the same problem: `cx + 1` produced the text `"[object Object]1"`, and Formula.js's IM* functions accepted complex numbers written as text while refusing the graph's own complex values.
...
**MUST:** every element-wise surface goes through `mapCells`, and the broadcast table and `broadcastRules.test.ts` change together, so editing either one alone fails. Two broadcasters are how the same expression ends up answering differently depending on where it's typed, which is the drift the parity program exists to close. Tying the table to the test is the one-implementation rule for gating metrics ([[engineering#A gating metric has one implementation]]) applied to semantics.
...
| `BYROW(a, λ)` | `(row as a list)` | a one-column table, one value per row beside the rows it came from ([[D85]] columnsStayColumns) |
...
| `MAKEARRAY(rows, cols, λ)` | `(row, col)`, 1-based | a rows by cols matrix, one column included ([[D85]] columnsStayColumns); a `null` dimension answers `null`; past `MAX_GENERATED` `#OVERFLOW!` |
...
**Rebuild.** `_rebuild()` recomputes the variables, adds an `anydata` input (scalar, list or matrix, never a Frame) for each new one, recompiles the evaluator and the AST, and returns the added and removed names. The single edit path, `applyExprChange` in `components/expressionEdit.ts`, is a no-op on a locked node; otherwise it sets `expr`, rebuilds, drops the removed sockets' cables before removing the sockets ([[input-cable-pruning#The ordering rule]]), resizes the card and recomputes the graph. A locked node is a pack preset: the formula is read-only and the title stays editable.
...
- **The regression quartet.** TREND(known_ys, [known_xs], [new_xs], [const]), GROWTH (the same, exponential), LINEST(known_ys, [known_xs]) answering `[slope, intercept, R²]`, and LOGEST(known_ys, [known_xs]) answering `[m, b]`. Pairs are prepared by `pairPresent`: an error is the answer, a pair with a blank side drops, and ragged tails are cut. Omitted Xs are 1 to n and omitted new Xs are the known Xs, the Excel defaults the node's sockets cannot express. TREND and GROWTH take Excel's `const`: FALSE fits through the origin (b = 0), or for GROWTH through b = 1, by least squares on ln y; a Y at or below 0 there answers `[]`. LINEST and LOGEST take neither `const` nor `stats`, since both wait on their answer's shape matching Excel's. A prediction target that is an error is the answer, and a blank target drops. A blank `known_ys` answers blank; a degenerate LINEST is blank (the node's three blank outputs); LOGEST with a Y at or below 0, and a TREND or GROWTH that cannot fit, answer `[]`.
- **INTERPOLATE** covers both of the Interpolate node's modes under one name ([[#Derived names are unique]]), chosen by the first argument's rank. List mode, `INTERPOLATE(known_ys, known_xs, new_xs)`, requires all three arguments (`#VALUE!` otherwise), prepares the known pairs with the node's `pairPresent`, and answers `interpolateLinear` per query; an error in the queries is the answer, a blank query stays blank in place, and a scalar query answers a scalar. Grid mode, `INTERPOLATE(table, [xs], [ys], [forecast])`, takes a matrix first and runs [[bordered-grid-fill]]. There a blank positional argument is an omitted axis counting 1, 2, 3, like an unwired socket, because a formula has no cables and so no wired blank; `forecast` defaults to TRUE.
...
- **XLOOKUP(lookup, keys, values, [if_not_found], [match_mode], [search_mode])** and **XMATCH(lookup, keys, [match_mode], [search_mode])** run `xmatchIndex`, the XMATCH node's kernel, on Solenoid's 1-D list model. `match_mode` is 0 (exact, also the default for an omitted argument), 1 (next larger) or −1 (next smaller); wildcard matching (2) is `#VALUE!` "isn't supported", and any other value `#VALUE!`. `search_mode` is 1 (first, the default) or −1 (last); binary search (±2) is `#VALUE!`, since every search scans the whole list. A blank mode argument reads as 0 (*Blank and omitted arguments*), which the search mode rejects as Excel does.
  ...
  - An array lookup value spills, as in Excel: one result per element, as a list. A 1 × N or N × 1 matrix lookup value spills over its cells; a true grid is `#SHAPE!`. This spill is scoped to the lookup family; it is not a general per-argument spill ([[#Argument routing]]).
...
- **EXPAND(array, rows, [cols], [pad_with])** grows the array; a blank row or column count keeps that axis's size ([[input-roles]]), and an omitted `pad_with` pads with blank, the author's override of Excel's `#N/A`.
...
- **TOCOL(array, [ignore], [scan_by_column])** stacks the cells into a one-column table and **TOROW** (same arguments) lines them up as a list (a row) ([[D85]] columnsStayColumns). Both read row by row unless `scan_by_column` is TRUE; `ignore` is Excel's code, 0 keeping every value, 1 skipping blanks, 2 errors and 3 both, and anything else is `#VALUE!`, as is a `scan_by_column` that isn't TRUE or FALSE. The Table Reshape card's TOCOL and TOROW ops carry the same two choices as its By row / By column toggle and its skip picker (`flattenCells`, `scanBy`, `skipCells`). TOROW is the way to turn a table or a column into a list.
...
- **SORT(array, [sort_index], [sort_order], [by_col])**, **SORTBY(array, by_array1, [sort_order1], …)**, **FILTER(array, include, [if_empty])** and **UNIQUE(array, [by_col], [exactly_once])** take a list or a table, as Excel's do; a list is one row ([[D85]] columnsStayColumns), and the answer is a list again while one row is left. SORT sorts the rows by the values in column `sort_index`, or with `by_col` the columns by row `sort_index`; an index outside the table is `#VALUE!`, so on a list SORT changes nothing without `by_col`. SORTBY reads each key's shape: keys that are one-column tables sort the rows, keys that are rows (a list is one) sort the columns, earlier keys first, each with its own order; keys that run different ways, or a key of another length, are `#VALUE!`. FILTER does the same with its mask: a column keeps rows, a row keeps columns, another length is `#VALUE!`, and an empty result answers `if_empty` when given. UNIQUE keeps the first of each distinct row (or column with `by_col`), and with `exactly_once` only those that appear once; on a list without `by_col` it changes nothing. A sort order other than 1 or −1 is `#VALUE!`, and the settings are declared roles, so a blank one is its default ([[input-roles]]). All sort through `compareListCells`: numbers by value, text by character code ([[C59]] byteStringOrder), then FALSE and TRUE, Excel's order across kinds; blanks, errors and NaN go last in either direction, in their original order. Each include cell is read as IF reads a condition (`ifTest`): text other than TRUE or FALSE is `#VALUE!`, a blank keeps nothing, and the first error in the include array is the answer, while an error in the data stays in its cell ([[E10]] pickVsAggregateErrors). The kernels are `sortGrid`, `sortGridByKeys`, `filterGrid` and `uniqueGrid` (`nodes/listOps.ts`), shared with the Sort, List Filter and UNIQUE cards.
- **TAKE** and **DROP(array, rows, [cols])** take Excel's signed counts through the one `takeSlice`/`dropSlice` kernel, per axis on a matrix. A list is one row ([[D85]] columnsStayColumns), so its items are columns: `TAKE(x, , 2)` is its first two items, `TAKE(x, 2)` is the whole list, `DROP(x, 1)` leaves nothing, and a list comes back as a list. Taking 0 rows or columns or dropping everything is `#DOMAIN!` (Excel's `#CALC!`), never a silent empty array. DROP of 0 drops nothing. A blank row count keeps every row (`TAKE(m,,2)`, `DROP(m,,1)`), which is what Excel's blank means in that slot ([[C80]] blankArgIsExcelBlank). The TAKE / DROP card has no way to leave a count out, so its 0 means "left out" and keeps the whole axis.
- **UNIQUE**, **MODE.MULT** and **FREQUENCY(data, bins)** answer blank for a blank argument.
- **RANDARRAY([rows], [cols], [min], [max], [integer])** defaults to one row, one column, 0 and 1. Its counts read as SEQUENCE's do (`arrayCount`, shared with the RANDARRAY card): they truncate, a negative one is `#VALUE!`, and zero answers an empty list. A Min above Max is `#VALUE!`, as in Excel. The whole-number option draws each whole number in [Min, Max] equally often, and a range holding none is `#VALUE!`; the card and the formula share `randArrayRange` and `randArrayDraw` (`listOps.ts`). It is volatile, fresh values each evaluation, while the node holds its rolls for a recalculation.
...
| `containsValue` | Membership by value (`setKey`, as under Sets below); blank and error cells are never members. Returns a boolean. |
...
**Sets.** Every set, dedupe, tally or membership test compares values by what they are, never by where they came from: two equal values from different nodes are the same value. `setKey` turns a complex value into a canonical string and leaves every primitive as itself. Blank and error cells equal nothing, so they are never members. Each of them keys through `setKey`; a JS `Set` or `Map` over raw values is a defect wherever a value might be an array or a tagged object, since it keys objects by reference, and a complex number is such an object ([[value-semantics#The value grammar]]): two equal complex numbers from different sources would never match (the Set node once had exactly this bug).
...
- `splitText` (TEXTSPLIT(text, col_delimiter, [row_delimiter], [ignore_empty], [match_mode], [pad_with])): an empty delimiter splits into characters. Without a row delimiter the answer is a list (one row); with one it splits into rows first and answers a table, short rows padded with `pad_with` or `#N/A`. `ignore_empty` drops empty parts; match_mode 1 matches case-insensitively.
- `textAfterBefore` (TEXTAFTER, TEXTBEFORE(text, delimiter, [instance_num], [match_mode], [match_end], [if_not_found])): the text after or before the `instance_num`-th occurrence of the delimiter, counted from the end when negative; 0 is `#VALUE!`. `match_end` 1 counts the end of the text (the start, from the end) as one more occurrence. An empty delimiter, or an occurrence the text does not have, answers `if_not_found` when given and otherwise null rather than Excel's `#N/A`. The formula's settings are declared roles ([[input-roles]]).
- *Diverges:* Excel reads an array of delimiters in these three as "any of them"; here the delimiter is one text, and TEXTAFTER and TEXTBEFORE, being element-wise, pair a list of delimiters with the texts item by item.
