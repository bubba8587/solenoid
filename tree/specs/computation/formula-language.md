...
- **TOCOL** stacks the cells into a one-column table and **TOROW** lines them up as a list (a row); both read row by row, as Excel's do and as the Table Reshape node does ([[D85]] columnsStayColumns). TOROW is the way to turn a table or a column into a list.
