...
**Sets.** Membership is by value ([[D39]] keyByValue): `setKey` turns a complex value into a canonical string and leaves every primitive as itself. Blank and error cells equal nothing, so they are never members. Every set, dedupe, tally or membership test keys through `setKey`; a JS `Set` or `Map` over raw values is a defect wherever a value might be an array or a tagged object, since it keys objects by reference and two equal complex numbers from different sources would never match (the Set node once had exactly this bug).
