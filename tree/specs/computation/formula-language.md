...
- **TOCOL(array, [ignore], [scan_by_column])** stacks the cells into a one-column table and **TOROW** (same arguments) lines them up as a list (a row) ([[D85]] columnsStayColumns). Both read row by row unless `scan_by_column` is TRUE; `ignore` is Excel's code, 0 keeping every cell, 1 skipping blanks, 2 errors and 3 both, and anything else is `#VALUE!`, as is a `scan_by_column` that isn't TRUE or FALSE. The Table Reshape card's TOCOL and TOROW ops carry the same two choices as its By row / By column toggle and its skip picker (`flattenCells`, `scanBy`, `skipCells`). TOROW is the way to turn a table or a column into a list.
