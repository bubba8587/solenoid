...
- **SORT(array, [sort_index], [sort_order])** works on one list: `sort_index` must be 1 or omitted (`#SHAPE!` otherwise), and order −1 sorts descending. **SORTBY(array, by, [sort_order])** refuses a key list of another length with `#SHAPE!`, as the Sort card and Excel do, and a sort order other than 1 or −1 with `#VALUE!`. Both sort through the List Sort card's `compareListCells`: numbers by value, text by character code ([[C59]] byteStringOrder), then FALSE and TRUE, Excel's order across kinds; blanks, errors and NaN go last in either direction, in their original order. Both answer the element type they were given (`returns: "any"`). **FILTER(array, include, [if_empty])** refuses an include array of another size with `#SHAPE!`, and an empty result answers `if_empty` when given. Each include cell is read as IF reads a condition (`ifTest`): text other than TRUE or FALSE is `#VALUE!`, a blank keeps nothing, and the first error in the include array is the answer, while an error in the data stays in its cell ([[E10]] pickVsAggregateErrors).
...
- A matrix given one index and no column walks it when it is one row or one column, as Excel's INDEX does on a range of that shape: `INDEX({1,2,3}, 2)` and `INDEX(TOCOL(x), 2)` are the second value. Any other matrix reads the one index as a row.
