...
- A flat list has no fixed orientation for positions ([[D84]] listEitherAxis): it is one row for its shape, but SEQUENCE(n), TOCOL and MAKEARRAY(n, 1) answer lists too. With one index, the index walks along it: `INDEX(x, 2)` is the second item and 0 is the whole list. With two, it reads along whichever axis names a position past 1: `INDEX(x, 1, 2)`, `INDEX(x, 2, 1)`, `INDEX(x, 0, 2)` and `INDEX(x, 2, 0)` are all the second item, `INDEX(x, 1, 0)` is the whole list, and a row and a column both past 1 is `#REF!`.
