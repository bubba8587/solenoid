...
- **The logical and number bridge.** On the number-family rungs (`number`, `list`, `numlist`, `table`) every boolean, at any depth, becomes 1 or 0 before shaping. On the logical-family rungs every number, at any depth, becomes `value !== 0`, and `NaN` becomes `null` (unknown, not true). `null` passes both ways. This is the only cross-family conversion coercion performs ([[D11]] noAutoCross): text or a complex on a number-family rung (only a wildcard cable can land one there) is `#TYPE!`: a cell of the list or matrix, or, as a lone value on `number` or `numlist`, the node's result ([[B17]] typedValueModel).
