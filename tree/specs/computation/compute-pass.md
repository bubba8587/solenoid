...
- **The shape helpers** (`nodes/coerce.ts`). `toMatrix` only widens and never fails: a scalar is 1×1, a list one row, a matrix unchanged. `toList` wraps a scalar, flattens a 1×N or N×1 matrix and throws `ShapeError` on a real M×N table. `toScalar` flattens and throws `ShapeError` on more than one element. A scalar is any value that is not an array, so text that reaches a number-family rung through a wildcard (`trueany` accepts everything) arrives as one value for the node to judge, never as a list of its characters. `toAnyMatrix` is `toMatrix` for any element type. `matrixShape(v)` is the one row and column count that the Table Info node's outputs and the COLUMNS and ROWS formulas share ([[C17]] shareImpl): a list is one row, a scalar is 1×1, and a wired blank is unknown (`null`). A Frame has its own real shape, so the node handles it before calling this, and Frames never reach formulas. The error guard recognizes `ShapeError` by its `name`.
