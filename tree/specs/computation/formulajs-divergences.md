...
- **MIN, MAX** are owned only because Formula.js spreads the whole list into `Math.min` and `Math.max`, which throws past about 125k values. They read what Formula.js reads: the numbers among the arguments, with text and logicals skipped, and no numbers is 0.
