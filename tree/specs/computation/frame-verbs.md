...
"Reads as a number" (`cellToNumber`) accepts a finite number, a boolean (as 1 or 0), a `UnitCell` (its display magnitude), or text that `decimalFromText` (`valueKinds.ts`) reads: trimmed plain decimal or scientific text, or thousands grouped by commas (`^[+-]?\d{1,3}(,\d{3})+(\.\d*)?$`), so the European `3,5` stays text instead of reading as 35. A `0x`, `0b` or `0o` radix prefix and `Infinity` are text, as in Excel. A column whose non-blank cells are all JS booleans infers logical before the number test runs; a mix of booleans and numbers reads as number with 1/0 cells. A 0/1 column stays number, because inference reads only the literals `TRUE`/`FALSE` as logical.
...
A `CubeColumn` is `{ name, cells, type?, format? }`. `type` is the column's element type. A declared type (Cube Input's column type, a Solenoid Properties pick, a source Frame column's type) holds every cell to it ([[D80]] cubeColumnTypes); one inferred for a column nobody typed is a display hint, not a promise that every cell matches. A list column's type is its items' type, so its chips tint and print by it. `format` is the Cube counterpart of `FrameColumn.format`: a producing verb can stamp a date pattern on a column (a Minutes-mode Schedule stamps `DD-MMM-YYYY HH:mm` on Start and Finish).
...
**Records to a Cube.** `recordsToCube(records, picks?)` is the rows-of-objects reader that frontmatter and the vault readers share. Columns are the keys in first-appearance order, named through `makeHeaders`. A list value becomes a list cell (never joined into text); a list whose present items are all objects becomes a nested Cube, a `null` beside the objects counting as an empty record; a single object becomes a one-row nested Cube. A picked column (from `picks`: Cube Input's typed columns, the Solenoid Properties plugin's `columnTypes`) is read through `typedCubeCell`, which overrides every kind inside it ([[D80]] cubeColumnTypes): a scalar crosses the type's own value boundary, `coerceFrameCell`, as a Frame Input cell would, so what the type cannot read becomes `NaN`, never a silent blank ([[D72]]); a list's items cross `coerceListItem`, List Input's reading, so what the type cannot read is blank (Date over `[10-Sep-2026, apple]` is a date and a blank); a nested Cube or Frame has every column set to the type and read the same way. The records are never rewritten. An unpicked column of scalars keeps its cells as they came and takes `inferColumn`'s type as its hint; an unpicked column of lists or tables carries no type.

**Cube Input's source.** `cubeText` is a JSON array of records while no column is typed or computed, and `{ columns, rows }` once one is (`cubeSourceToText`). `parseCubeSource` reads either into a `CubeSource`: `columns` lists every column in order, each `{ name, type?, expr? }` (a type outside the four is none; a repeated or blank name is dropped), with any record key the list lacks appended in first-seen order, and `rows` are the records as typed. `cubeFromSource` builds the typed columns through `recordsToCube` with the declared types as picks, places every column in the list's order, then fills the formula columns ([[computed-columns]]). Blank text is an empty cube; anything else that is not one of the two forms is one `#VALUE!` with its reason. An unchanged text returns the same cube object.
...
Cells cross as JSON. Upload direction (`encodeWireCell`): a finite number, string, boolean or `null` as is; a non-finite number as `{"__nf": "inf" | "-inf" | "nan"}`; a SolError as `{"__err": code, "ref": n}`, where `ref` names the original error in a registry of weak references (`errorRef`), so the cell downloads as that same SolError, message and origin included, while the original is alive. Only name, type and values cross: `unit`, `format` and `raw` are not sent and do not come back. The Polars backend puts `unit` and `format` back itself: each handle keeps a row-less copy of its frame's columns, every verb that makes a new handle is run over those copies with the same JS verbs, and a collected, previewed or read column takes the unit and format of the copy with the same name and type. So a unit survives exactly the verbs it survives on the web. `raw` is not restored. A unit changes values in two places, readings on an offset scale and a variance over a scaled linear unit, so before sending, `lowerForEngine` (`frameBackend.ts`) runs each op against the copy it will meet and writes the column's `readingScale` onto every groupBy aggregate and window over readings, and its `unitScale` onto a `var` or `varp` over a scaled linear unit (`withUnitScales`). The engine coerces each cell by the declared column type (`json_to_cell`):
...
Download direction (`num_to_json`): an integral number within the safe range as a JSON integer, other finite numbers at shortest round-trip precision, `±Infinity` and a canonical `NaN` as the `__nf` sentinel, an uploaded error cell as `{"__err": code, "ref": n}`, and an error the engine computed as `{"__err": code, "why": reason}`. The JS side decodes both sentinels back (`decodeWireCell`): a `ref` still in the registry returns the original SolError, and a computed error takes the message the oracle gives for its reason (`ENGINE_ERROR_WHY`), so a cell reads the same on both engines; the corpus compares error cells by code alone. A failed command rejects with `{ __solError: true, code, message }` (`IpcError` in `ipc.rs`), the same shape as the web side's tagged `SolError`, so a failure crossing IPC arrives as a value the app already renders and IFERROR can catch. `toSolError` accepts only canonical codes and maps anything else to `#ERROR!`. An unexpected engine failure is `#ERROR!` (`IpcError::internal`). `engine_ping` returns `{ name: "solenoid-engine", version, backend: "polars" }`. `engine_drop` is fire-and-forget: its failure is ignored.

A string or logical column cannot hold an error cell natively, so a frame holding one never uploads: `PolarsBackend.source` keeps it on an embedded oracle backend (`jsf:` handles), every unary verb on it runs there, and a join, append or bind columns that meets one collects its native partners into the oracle first. A column's source text (`raw`) is gone after any native verb. The reason: [[C16]] polarsEngine promises identical answers from both engines, and Polars' string and boolean columns have no bit pattern to spare for an error, so uploading one would turn the error into a blank; routing only those frames keeps answers exact and leaves the common case (errors in computed numeric columns and date parsing) native. Such a frame runs at oracle speed. Revisit if those frames get large enough to be slow, or if the engine gains a lossless error encoding for text and logical columns (a reserved string, or a side column of codes).
...
Why the lock is handled this way ([[C16]] polarsEngine): a panic deep in Polars would otherwise poison the lock and fail every later engine call until the app restarts, while the webview keeps running, and one long verb held under the lock would stall every other call. The store is only a handle map, so its data is valid wherever a panic unwound.

A number or date column holds an error cell as a reserved quiet-NaN bit pattern. An uploaded error is tagged `0x7ffc` in the top 16 bits, with its code's index in `ERR_CODES` in bits 32 to 47 and its `ref` in bits 0 to 31. An error the engine computes has one pattern per reason: `#DOMAIN!` (`0x7ff8_0000_0000_0d01`), `#OVERFLOW!` (`…0f02`), `#DIV/0!` for a percent change from zero (`…0d03`) and for a zero group total (`…0d06`), and `#UNIT!` for a sum of readings (`…0e04`) and for a reading scaled (`…0e05`). A genuine data NaN is always the canonical `0x7ff8000000000000`, so the patterns cannot collide with real values. `error_of` reads a cell's code off its bits, and every verb reads a reserved pattern as the oracle reads a SolError: sort tails it, a comparison, `eq` and `neq` included, drops it, `iserror` keeps it (`is_err_expr`), a join key never matches it, Distinct and the groupBy and window keys tell errors apart by code (`key_json`, `nonfinite_label_expr`), an aggregate or a window function over a partition holding one answers with the first in row order (`first_error_or`), and fill and lag carry it as a value. Moving rows keeps the bits, and no verb does arithmetic on an error cell.

### The parity corpus

Every verb in `FRAME_OP_KINDS` and the binary verbs `join`, `append`, `bindColumns` has a fixture file in `fixtures/frame-verbs/<verb>.json`, plus `pipeline.json` ([[#The parity corpus]]). A file is `{ verb, cases: [{ name, frames, op, expect | expectError }] }`: `frames` maps names to wire frames (`in` for unary verbs, `left`/`right` for join, any names listed in `op.frames` for append and bind columns), `op` is the wire op (for join `{ kind: "join", …JoinOpts }`, for a pipeline `{ kind: "pipeline", ops }`), `expect` is a wire frame and `expectError` an error code. Exactly one of `expect` and `expectError` is present; case names are unique per file.

`scripts/fuzz-frame-verbs.ts` writes random cases for every unary verb, pipelines, join (cross and as-of over number and date keys included), append and bind columns as `fuzz-*.json` files with the oracle's answers. Its frames carry error cells in number and date columns, its filters the error predicates, its groupBy aggregates and windows `readingScale` and `unitScale`, and its groupBy may have no keys; `cargo test corpus_cases` then hunts divergences, and a find is kept as a hand-named case. `frameVerbCorpus.test.ts` runs each case through the oracle (a pipeline applies its ops one after another); `corpus_cases` in `src-tauri/src/engine/tests.rs` deserializes the same files with the production `WireFrame`, `WireOp` and `WireJoinOpts` types and runs them through the engine (a pipeline through `apply_ops`, fused). Comparison is on name, type and values; `-0` compares equal to `0` and error cells compare by code. `ORACLE_ONLY_VERBS` (`["pivot"]`) makes the cargo runner assert that the op still does not parse as a `WireOp`. The completeness test fails if a verb in the inventory lacks a file or a file names a verb outside it; `FRAME_OP_KINDS` is checked against the `FrameOp` union at compile time.

**MUST:** every verb both engines speak is specified by corpus cases, and a verb without them doesn't ship. A verb only one side runs is declared (`ORACLE_ONLY_VERBS`), never skipped, so the exemption removes itself if the engine learns the op. **MUST NOT** re-encode a case per side: the corpus format is the wire format, so a fixture that parses on one side and not the other is itself the parity failure, surfacing at load.

The two engines were once kept in agreement by hand-mirrored test pairs tied together by comments, and nothing failed when a verb, option or edge case landed on one side only ([[C16]] polarsEngine). The corpus's first runs each caught a divergence the pairs had missed: a silent null column for an unknown aggregate op (now `#NAME?` on both sides), NaN passing gt and gte under Polars' total float order, and outer-join row order. Extending the fuzzer to window, fillBlanks, replaceValues, sliceRows, bindColumns and cross join found eight more in one run, each pinned as a shared wire case.
...
A text predicate (`contains`, `startsWith`, `endsWith`) on a column that is not string is `#TYPE!` ([[D49]] textPredicateNeedsText), and the message names the column, its type and the two fixes: a Computed Column such as `TEXT(@<col>, "@")`, or Cast to Text. The List Filter and the *IFS criteria use the list twin, `requireTextList`. One such condition fails the whole filter, under `or` too. There is one gate on each side of the engine seam: `requireTextColumn` / `requireTextList` in `frameVerbs.ts` and `require_text_column` in `engine.rs`, called by every filter entry (the Filter verbs, List Filter, the *IFS criteria). No filter path ever compares `String(cell)` of a number, date or logical, so results never depend on JavaScript's number printing and the native engine never has to mirror it.
...
`{ kind: "groupBy", keys, aggs: [{ column, op, as }] }`. One output row per distinct key tuple (keyed by `encodeCell`), in first-seen order; a null key is its own group, and each non-finite key value and each error code is its own group. With no keys the whole frame is one group, and a frame with no rows has none. Output columns are the key columns (type and unit kept, format dropped), each holding the group's first-seen cell, then one column per aggregate named `as`. All output names go through `makeHeaders`, so an aggregate named after a key becomes `k2` and the key keeps its name. An unknown key or aggregate column is `#REF!`.
...
**Readings on an offset scale** (a °C or °F column) answer as a formula does ([[C25]] firstClassUnits, [[unit-flow]]); `aggUnitPlan` in `frameVerbs.ts` holds the rule. `sum`, `product` and `percentof` are `#UNIT!` in every group, since the op, not a cell, is at fault, and the column carries no unit. `stdev` and `stdevp` are a delta in kelvin and `var` and `varp` a squared one: the value scales by the unit's scale per power (5/9 for °F) and the column unit is the bare temperature dimension. `count` is plain; the rest stay readings in the column's unit. The op carries the scale as `readingScale` on each aggregate; the oracle reads it off the column's unit when absent. Over a linear unit, `stdev` and `stdevp` keep the column's unit and `var` and `varp` square it, landing in base SI as a formula's VAR does (a km column's variance is in m², the value scaled by the unit's scale squared, carried to the engine as `unitScale`).

`aggregateGroup` is exported: Cube Rollup (`cube.ts`) runs it and `aggUnitPlan` over a nested sub-frame's column, passing that column's type and unit, so a roll-up and a GROUPBY agree on every op's edge cases and units. A nested cube's column reads its tagged cells in one unit first (`matrixCellsFromList`); when it carries no type, `inferColumn` reads its non-error cells for one, as a cube-to-frame read does, and its error cells stay errors. A roll-up that produced any text cell is a string column; otherwise it is number, carrying the unit its rows agree on. The Window verb's `group_min`, `rolling_min`, `cummin` and their max twins do not share it and stay numeric.
...
**Sorting.** `rowSort` and `colSort` are signed 1-based indices into `[fields…, values…]`; negative means descending. A field index re-ranks that level by value (the column type's comparator, blanks and errors last), and descending reverses that level's whole order, which puts blanks first. A value index reorders the leaves by that value's aggregate over the whole axis (a `percentof` scores by sum; a text `min` or `max` scores by code unit, [[D76]] textMinMax), and a blank, `NaN` or error score sorts last, and only when the axis has more than one leaf; it reorders leaves flatly.
...
`{ kind: "fillBlanks", columns, dir: "down" | "up" }`. In each listed column (every column when the list is empty), a `null` cell takes the nearest present value above (`down`) or below (`up`); a leading (or trailing) run stays blank. An error cell is a value: it is never filled, and it fills the blanks after it like any other. An unknown column is `#REF!`.
...
- `cell`: a whole cell matches when a number (or date serial) equals `decimalFromText(find)` (a non-numeric find matches no number), a logical's `TRUE`/`FALSE` equals `find` in upper case, or a string equals `find` exactly. Blank and error cells never match. The replacement is coerced to the column type: blank text becomes `null`; a number or date column needs a finite number, and otherwise that column is left unchanged; a logical takes `true`/`1` or `false`/`0` (any case), anything else `null`; a string column takes the text verbatim. So Replace Values never writes a `NaN` into a number column, since a `NaN` cell is neither a value, a blank nor an error that anything downstream could read.

The card's Find and Replace take a wired value of any type, stringified by `readFilterValue`.

### join

`joinFrames(left, right, { leftKey, rightKey, how, asofDirection?, asofTolerance?, rightKeyScale?, rightKeyOffset? })`. The key columns must exist (`#REF!`) and share a type (`#TYPE!`, `Join keys must share a type ("<l>" vs "<r>")`); a Cross join needs neither. Keys of two types could never match, since element families never cross on their own ([[C10]] socketLattice), so the join refuses instead of returning a silent empty result.

Keys with units compare as quantities ([[C25]] firstClassUnits): `5 km` matches `5000 m`. When both key columns carry a unit and the units differ, `joinKeyTransform` reads the right key in the left key's unit as `right × scale + offset` (the offset is for temperatures). It treats each display unit as affine to base SI, `base = a·x + b`, reading `a` and `b` off the unit's own scale and offset; then `scale = a_right / a_left` and `offset = (b_right − b_left) / a_left`. The converted key rounds half up at the 15th significant digit of the larger of `right × scale` and `offset` (0 when it falls below that digit), so conversion noise cannot block a match: 68 °F matches 20 °C, 32 °F matches 0 °C and 0.3048 m matches 1 ft (`convertKey`, `convert_key` in the engine, which reads the exact decimal expansion as JS `toPrecision` does). The transform is applied to a number key only, and not at all when it is the identity. Keys that measure different things, or two different currencies, are `#UNIT!` (`Join keys measure different things (<l> and <r>). Convert one key first`). A unit on only one side converts nothing. Explicit `rightKeyScale`/`rightKeyOffset` override the derived transform; the native engine never sees units, so `PolarsBackend.join` derives the transform from its schema shadows and always sends it. The join corpus's `unit keys` cases carry the transform it sends.
...
3. The lookup arrives as text and is parsed by the key type: logical through `coerceLogical` (unreadable is false), date as a serial when it is plain digits and otherwise through `parseDateToSerial`, number through `decimalFromText`, string as is.
