...
`aggregateGroup` is exported: Cube Rollup (`cube.ts`) runs it over a nested sub-frame's column, passing that column's type, so a roll-up and a Group By agree on every op's edge cases. A roll-up that produced any text cell is a string column; otherwise it is number. The Window verb's `group_min`, `rolling_min`, `cummin` and their max twins do not share it and stay numeric.
