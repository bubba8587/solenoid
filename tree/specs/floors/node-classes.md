...
- **Frame input labels and their example hints** ([[C13]] frameLabelGrammar, [[D18]] frameLabelHint). A role chain joins its roles with `" + "`. A role-labeled input declares its example in the class's static `frameHints` map under the input key (`frameHint.ts`), and the hint's column names equal the label's roles in order. The λ-table inputs in `nodes/tableLambda.ts` are the one exception to the label grammar, scoped to that file.
