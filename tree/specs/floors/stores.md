...
3. **Document state lives in the saved text form, never in a store alone** ([[C30]] saveViaTextForm). A store that must survive a reload mirrors a persisted field: saving reads the store into the field, and the load path writes it back (`nodeSizeStore`, `collapseStore` and `socketFlipStore` from a saved node's `size`, `collapsed` and `flipped`, on the main canvas and inside a composite's `init.internal` alike (`savedNodeBody.ts`); `pinStore` and `commentStore` through `serialize()` and `load()`, which the load path calls after remapping node ids). A store bound to the main graph does not leak into a composite drill-in ([[C33]] saveBindsMain). Only app preferences, not document state, go straight to `localStorage`, under a `solenoid.` key, with every read and write wrapped in try/catch so a private window still works.
