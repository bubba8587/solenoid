...
- **A node wrapper's inline `visibility`.** RF writes `visible` onto `.react-flow__node` after measuring, so any imperative visibility write there is silently overwritten. Per-node hide and show rides RF's `className` instead (`flowModel.nodeClassName` plus an `!important` rule in flow.css). Hiding the members of a collapsed group is the standing example ([[group-collapse]]).
...
- A Conduit's socket tips come from `conduitLaneOffset`, not from the measured handle ([[resizable-content-nodes]]).
...
Two mechanisms keep loads fast ([[graph-load-teardown-performance]]). The topology pipe merges a rebuild into one React commit. `syncTopology` keeps each surviving node's object identity, so RF's memo skips cards that didn't change.
