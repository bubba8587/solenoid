...
- **Hidden nodes.** Every member is hidden, along with any FC docked to a member (the group's "extended members").
...
- **Conduit bundles.** Outputs of a hidden Conduit that land on one external destination share one combined row and pill, so their cables draw as a single ribbon. Inbound cables from one external Conduit likewise share one left pill.
