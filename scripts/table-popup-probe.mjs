// Measures the DOM cost behind table and cube popup virtualization: building 1000 rows (the popup's
// MAX_VISIBLE_ROWS) of read-only <input>, editable <input> or plain-text cells at 3, 10 and 30 columns,
// a value-only retouch (the memoized-keystroke floor) and a full tbody rebuild (what an unmemoized
// keystroke costs). Runs on the dev-server page (:1420) so the app's own CSS drives layout.
      ...
      for (const cols of COLS) {
        for (const variant of ["ro", "rodiv", "text"]) {
          ...
          for (let rep = 0; rep < REPS; rep++) {
            ...
            void host.offsetHeight; // forces a synchronous layout
