...
Formula.js's array functions are written against 2-D ranges with unvetted quirks, and it has
been caught changing its arguments in place (CHISQ.TEST). The original rank cap was partly
containment, and at rank 2 that logic stays even though the cap itself lifted ([[C15]]
matricesInFormulas). The complex half has the same shape: before the gate, `cx + 1`
concatenated to `"[object Object]1"`, and Formula.js's IM* functions worked on text complexes
while refusing the graph's own tagged values.
