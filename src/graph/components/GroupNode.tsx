...
export function GroupComponent({ data, emit }: NodeProps<GroupNodeType>) {
  ...
  function onResizeDown(e: React.PointerEvent) {
    ...
    const move = (ev: PointerEvent) => {
      node.width = Math.max(140, startW + (ev.clientX - startX) / k);
      node.height = Math.max(90, startH + (ev.clientY - startY) / k);
    ...
    const up = () => {
      ...
      if (gridSnapStore.get()) {
        ...
        if (pos) {
          node.width = Math.max(140, snapCoord(pos.x + node.width) - pos.x);
          node.height = Math.max(90, snapCoord(pos.y + node.height) - pos.y);
          void area?.update("node", node.id);
        }
      }
