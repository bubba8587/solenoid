...
/** Rendered in NodeShell's non-scrolling cornerBadge slot, so it stays pinned over a body that scrolls when the card is sized down. */
export function ValueExpandButton({ value, label, elem }: {
  value: unknown;
  label?: string;
  /** Forwarded so a matrix popup gets its format and unit row; ignored for a Frame. */
  elem?: ElemFamily;
}) {
