...
import { TDistNode, ChisqDistNode } from "./nodes/dist-normal";
import { FDistNode, GammaDistNode } from "./nodes/dist-continuous";
describe("registered gap distributions match the NODES", () => {
  ...
  it("T family", () => {
    ...
    close(ev("T.INV(0.95, 10)"), new TDistNode({op:"inv"}).data({prob:[0.95],df:[10]}).result);
    close(ev("T.INV.2T(0.05, 10)"), new TDistNode({op:"inv2t"}).data({prob:[0.05],df:[10]}).result);
  });
  it("CHISQ.RT + F.RT", () => {
    close(ev("CHISQ.DIST.RT(8, 5)"), new ChisqDistNode({op:"rt" as never}).data({x:[8],df:[5]}).result);
    close(ev("CHISQ.INV.RT(0.05, 5)"), new ChisqDistNode({op:"invrt"}).data({prob:[0.05],df:[5]}).result);
    close(ev("F.DIST.RT(3, 5, 10)"), new FDistNode({op:"rt" as never}).data({x:[3],df1:[5],df2:[10]}).result);
    close(ev("F.INV.RT(0.05, 5, 10)"), new FDistNode({op:"invrt"}).data({prob:[0.05],df1:[5],df2:[10]}).result);
  });
  it("GAMMA", () => {
    ...
    close(ev("GAMMA.INV(0.5, 2, 3)"), new GammaDistNode({op:"inv"}).data({prob:[0.5],alpha:[2],beta:[3]}).result);
  });
});
