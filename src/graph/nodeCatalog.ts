      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              { type: "decision-matrix", label: "Decision Matrix", description: "Scores and ranks a Frame of options: rows are options, number columns criteria, an optional leading text column names them. Score = Σ(value × weight) / Σ|weight|, then competition rank on the rounded score. A negative weight penalizes a lower-is-better criterion such as cost. Each criterion normalizes Raw, ÷Max (the default), or Rank so incompatible scales compare. Breakdown adds each criterion's signed contribution; the contributions sum to the Score. Output: Option · Score · Rank, best first.", create: () => new DecisionMatrixNode(), parity: false, keywords: "decision matrix weighted score rank ranking criteria weight choose compare options podium dmbv multi-criteria mcda" },
