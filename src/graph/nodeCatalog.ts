    ...
    children: [
      ...
      { type: "na",            label: "NA",          description: "Outputs #N/A — propagates through calculations like Excel. Catch it with IFERROR / IFNA.", create: () => new NaNode() },
      {
        ...
        children: [
          ...
          { type: "data-feed",     label: "Data Feed",   description: "Live economic & market data → a Frame: FRED economic series (no key needed) and stock history via Alpha Vantage (free API key). Wire it into a Chart to embed a FRED graph. Stores the series id / ticker, not the data — refresh to re-pull. Desktop for arbitrary URLs; the browser is CORS-limited.", create: () => new DataFeedNode(), parity: false },
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "gauge",     label: "Gauge",     description: "Show a value as a percentage on a radial dial (1 = 100%, 1.5 = 150%) — a speedometer-style readout. Pass-through.", create: () => new GaugeNode(), parity: false },
