...
export const NODE_CATALOG: CatalogEntry[] = [
  ...
  // ── CONTROL ──────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "color-picker", label: "Color", description: "Pick a colour in RGB or HSV and choose the output format: hex, rgb(), or hsl(). Outputs a CSS colour string.", create: () => new ColorPickerNode(), parity: false },
  ...
  // ── TABLES & FRAMES ───────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          { type: "map-table",  label: "MAP",       description: "Apply a formula to every cell of up to three same-shaped tables. Variables x, y, z = each table's cell; r, c = 1-based position (a scalar y/z broadcasts). Pick the result type for text/date. Excel: MAP.", create: () => new MapTableNode(),  parity: false },
      ...
      },
      {
        ...
        children: [
          ...
          { type: "head",        label: "Head",        description: "Keep the first N rows of a Frame. Wire or type the row count.", create: () => new HeadNode(), parity: false },
          ...
          { type: "filter-frame",label: "Filter Rows", description: "Keep a Frame's rows passing condition rows (column + test + value) combined with AND/OR; the SQL WHERE. Blanks/errors drop. Text tests ignore case like Excel's =; Match case per condition. Excel: FILTER.", create: () => new FilterFrameNode(), parity: false },
