    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "local-file",   label: "Local File",  description: "Loads a Frame from your data folder (Settings ▸ Data). A `.parquet` reads straight into the native engine, so typed columns arrive intact with no inference. A Project XML, GanttProject or Primavera XER file, or a CSV whose Predecessors use row numbers (3FS+2d), also comes out as a Plan: tasks nested as the outline with their predecessors resolved to names, ready for Schedule. Anything else is read as CSV with columns auto-typed. Stores the file name. Refresh to re-read. Desktop only.", create: () => new LocalFileNode(), parity: false, keywords: "csv parquet arrow column columnar native engine polars file load import project xml mspdi plan smartsheet gantt gan primavera xer" },
          { type: "pair", children: [
          ...
          { type: "write-file",    label: "Write File",  description: "Writes to a file: a Frame as CSV or as JSON rows, or Text, which writes the text it is given as-is, so the file is whatever it holds (name it plan.xml and it writes XML, the pair for Schedule's Project XML). Pick the format, arm it, then press Run. Never writes on its own. A normal recompute only updates the preview. Desktop only.", create: () => new WriteFileNode(), parity: false, keywords: "csv json text write export save file sink xml mspdi verbatim string" },
          ...
          {
            type: "category", label: "Obsidian", description: "Your vault as data and back: notes and TaskNotes tasks in, notes and properties out. Set the vault in Settings ▸ Obsidian. Desktop only.",
            children: [
              ...
              { type: "tasknotes", label: "TaskNotes", description: "Reads the TaskNotes plugin through its local HTTP API. Tasks gives every task as one row of a Cube: path, title, status, priority, due, scheduled, estimate and tracked minutes, then projects, contexts, tags and blocked-by as lists, time entries and completed instances as nested tables, and each user field as its own column. Calendar gives the events between two dates as a Frame. Stats gives the task counts as a Status and Count table. Turn the API on in the plugin's settings; set the address in Settings ▸ Obsidian and the token on the card.", create: () => new TaskNotesNode(), parity: false, keywords: "tasknotes task notes obsidian plugin api tasks todo due scheduled projects calendar events stats time tracking" },
              { type: "write-obsidian", label: "Write to Obsidian", description: "Writes a Note, a Report, or a cube of rows into your Obsidian vault. A Note or Report becomes one portable-markdown note (frontmatter, tables, mermaid, math, rasterized chart and image assets) in a vault subfolder, and a mail-merge Report writes one note per page. A cube of rows writes each note's frontmatter, and a note-body column its body, keyed by a path column and patched line by line so the rest of each note stays as it was. Auto picks by what you feed it, or pick Note or Properties. Preview shows what Run would do; arm, then Run. Never writes on its own. Vault: Settings ▸ Obsidian. Desktop only.", create: () => new WriteObsidianNode(), parity: false, keywords: "obsidian vault markdown md note export sink write document properties frontmatter yaml note-body cube patch bases plan preview" },
    ...
    children: [
      ...
      { type: "format-controller", label: "Format", description: "Sets a docked socket's number format (decimal, fraction, %, currency…) and a unit label like `°C`, `m`, or `kg`. Units must match on connected cables.", create: () => new FormatControllerNode() },
      {
        ...
        children: [
          ...
          { type: "gantt",     label: "Gantt",     description: "Draws a scheduled project as a Gantt chart: a bar per task along a time axis, milestones as diamonds, nested tasks as summary brackets, dependency arrows between them, and the critical path picked out. It reads the rows a Schedule node worked out, fills each bar to its percent complete, and can show a baseline behind the bars. A time scale from days to years, a today line, and shaded non-working days.", create: () => new GanttNode(), parity: false, keywords: "gantt chart project schedule timeline plan tasks bars milestones critical path dependencies predecessors baseline waterfall roadmap pert cpm" },
      ...
      { type: "pair", children: [
        ...
        { type: "report", label: "Report", description: "A standalone markdown document, separate from the graph, written as a Knap template. A bare `{{ name }}` embeds the wired value as the canvas shows it: a formatted number, a table, a chart, a typeset formula, or a Note whole. `{% for row in table %}` repeats over a frame, `{% if %}` gates a section, and filters such as `date`, `join` and `number_format` shape the text. A Note on Template becomes the text instead, its tags the inputs, its frontmatter the defaults. Records makes it a mail merge: one page per row of a frame, `record` and `index` in scope, each named by the page name.", keywords: "mail merge merge fields letters one note per row batch template document markdown knap", create: () => new ReportNode(), parity: false },
      ]},
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { type: "schedule", label: "Schedule", description: "Works out when each task in a project starts and finishes from its duration and what it waits on: the critical-path method. Rows are tasks; Task is the first text column, Duration the first number column in days (blank or 0 is a milestone), and Predecessors a list cell naming the tasks that must finish first. Links takes a flat frame of dependencies instead (Successor, Predecessor, Type, Lag), one row per link, so a plain tasks table with no list column can still carry them. Working days skips weekends and the Holidays list; Calendar days counts every day. Output: the rows with Start, Finish, Float and Critical appended, where Float is how many days a task can slip without moving the finish and Critical marks the tasks with none. One path marks only the chain that drives the project finish critical; Every path marks each independent chain critical on its own. Project finish is the last finish, and Gantt is Mermaid source a Mermaid node draws, with one section per Project column value.", create: () => new ScheduleNode(), parity: false, keywords: "schedule cpm critical path gantt project plan timeline tasks predecessors dependencies float slack milestone duration working days finish date pert multiple critical paths" },
              { type: "earned-value", label: "Earned Value", description: "Measures a project against its plan and its budget. Feed a scheduled project with a Cost column and how far each task is Complete, plus the baseline it started from; as of a status date it works out the planned value, the earned value and, where you track it, the actual cost, then the schedule and cost variances, the SPI and CPI indices, and the estimate at completion. Output: a row per task with those figures, and the whole-project SPI, CPI and EAC as their own values. Cost keeps its currency.", create: () => new EarnedValueNode(), parity: false, keywords: "earned value evm bcws bcwp acwp planned earned actual cost variance schedule variance spi cpi eac vac tcpi budget baseline project performance" },
              ...
              { type: "settle",      label: "Group Cost Settle", description: "Squares up a group that paid unevenly: who pays whom in the fewest transfers (From · To · Amount) plus each person's true cost: Paid, what they still Owe the group, what is Owed back to them, and Net (the fair share). Biggest creditor takes from biggest debtor, exact to the cent. Totals mode reads one row per person (what each Paid, optional Share weight, a couple as 2, a child as 0.5). Transactions mode reads a cube ledger of expenses, each with an Amount, who Paid (one name or a shared bill), and who it is For, split equally, where blank means the whole group, so a bill one person fronts can be redistributed to a different group.", create: () => new SettleNode(), parity: false, keywords: "settle settlement split bill trip expenses transactions ledger who owes whom debts transfers splitwise group cost share even up payer participants" },
            ],
          },
        ],
      },
