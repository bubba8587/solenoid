      ...
      },
      {
        ...
        children: [
          { type: "pair", children: [
            ...
            { type: "list-running", label: "Running", description: "One aggregate per element: `SUM` / `AVERAGE` / `MIN` / `MAX` / `MEDIAN` / `PRODUCT` / `STDEV` of everything so far (the running total) or, with a window of N, of the last N (the moving average).", create: () => new RunningNode(), keywords: "running total cumulative rolling moving average sliding window prefix sum accumulate expanding cumsum min max median product stdev" },
          ]},
          { type: "pair", children: [
