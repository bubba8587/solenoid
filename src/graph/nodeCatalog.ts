    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "computed-column", label: "Computed Column", description: "Adds a column computed row by row from a formula whose variables are the frame's column names — plus row, rows, and col(\"Unit Price\") for names a variable can't spell; a λ input carries one reusable formula across frames   (Power Query: Custom Column)", keywords: "custom column calculated field formula derive mutate row-wise index", create: () => new ComputedColumnNode(), parity: false },
        ],
      },
      {
