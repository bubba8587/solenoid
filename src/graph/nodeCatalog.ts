...
import {
  ...
  GroupNode, NoteNode, ReportNode, SessionHistoryNode, ImageNode,
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "join",        label: "Join",        description: "Combine two Frames on a key column — inner / left / right / outer. A left row matching several right rows fans out. The relational JOIN (Excel: VLOOKUP/XLOOKUP do a left join one column at a time).", create: () => new JoinNode(), parity: false },
          { type: "frame-lookup",label: "Frame Lookup", description: "Look up a value in one column of a Frame and return the matching cell from another column — VLOOKUP / XLOOKUP for tables. First exact match; If-not-found falls back, else #N/A (Excel: =XLOOKUP / =VLOOKUP).", create: () => new FrameLookupNode(), parity: false, keywords: "xlookup vlookup lookup frame table match find" },
