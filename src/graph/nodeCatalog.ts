    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "write-file",    label: "Write File",  description: "Writes a Frame to a file: CSV, JSON as an array of row records, or a Markdown table. A Note or Report on Document writes as Markdown, and a mail merge writes one file per page into the path as a folder. Pick the format, arm it, then press Run. Never writes on its own. A normal recompute only updates the preview. Desktop only.", create: () => new WriteFileNode(), parity: false, keywords: "csv json write export save file sink" },
          ...
          ]},
          {
            ...
            children: [
              ...
              { type: "write-obsidian", label: "Write to Obsidian", description: "Writes a Note or Report (its Document output) into your Obsidian vault as portable markdown: frontmatter, tables, mermaid, math, and rasterized chart/image assets, under a vault-relative subfolder. A mail-merge Report (Records wired) writes one note per page. Arm, then Run. Never writes on its own. Vault: Settings ▸ Obsidian. Desktop only.", create: () => new WriteObsidianNode(), parity: false, keywords: "obsidian vault markdown md note export sink write document" },
