...
export const NODE_CATALOG: CatalogEntry[] = [
  // ── INPUT ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "list-input",  label: "List Input",    description: "Builds a list from comma-separated values (e.g. 1, 2, 3) in each row, or wire a list into a row; every row concatenates into one output list. Element type: number, text, date, or TRUE/FALSE. Excel: selecting a range like A1:A8.", accent: NODE_KIND_ACCENTS.list, keywords: "literal array csv combine concat number text string date boolean logical type", create: () => new ListInputNode() },
      ...
      { type: "lambda-make",   label: "LAMBDA",      description: "Defines a reusable formula as a value. Parameters go in the λ(…) row; other variables become captured inputs. Feeds MAP / BYROW / REDUCE / MAKEARRAY; parameters bind positionally. Like Expression, the formula evaluates via Formula.js with standard Excel functions, separate from the visual nodes. Excel: LAMBDA.", accent: NODE_KIND_ACCENTS.lambda, create: () => new LambdaNode(), parity: false },
      ...
      },
      {
        ...
        children: [
          ...
          { type: "write-obsidian", label: "Write to Obsidian", description: "Writes a Note or Report (its Document output) into your Obsidian vault as portable markdown — frontmatter, tables, native mermaid blocks, math, and rasterized chart/image assets. A vault-relative subfolder; arm it, then press Run. Never writes on its own. Set the vault in Settings ▸ Obsidian. Desktop only.", create: () => new WriteObsidianNode(), parity: false, keywords: "obsidian vault markdown md note export sink write document" },
        ],
      },
    ],
  },
  ...
  // ── OUTPUT ───────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "alert",     label: "Alert",    description: "Watch a value and fire a toast plus an Alerts HUD entry on a status change. Modes: range with Low/High thresholds, boolean where TRUE fires, change on any new value, or threshold-cross.", create: () => new AlertNode() },
      {
        ...
        children: [
          { type: "expect", label: "Expect", description: "Data Validation, generalized. Checks a value against opt-in rules: not-null, unique for a list, in range, matches a regex, or in an allowlist. Always pass-through: a failure never blocks the value. A failing check shows a red badge and fires an Alert once per new failure.", create: () => new ExpectNode(), parity: false, keywords: "expect validate validation data quality check rule assert not null unique range regex allowlist in list membership enum whitelist trust" },
          { type: "tornado", label: "Tornado", description: "One-at-a-time sensitivity ranking. Any numeric value, then Run: it perturbs each upstream Number/Slider input ±10% (or its declared min/max) one at a time, reads how much THIS node's value swings, and ranks the inputs by impact in an inline tornado chart. Pass-through.", create: () => new TornadoNode(), parity: false, keywords: "tornado sensitivity analysis what-if one at a time impact ranking swing trust" },
        ],
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              { type: "treemap",   label: "Treemap",   description: "Shows labelled values as nested rectangles sized by value, a space-filling alternative to a pie. Takes a 2-column frame (label, value).", create: () => new TreemapNode(), parity: false, keywords: "treemap tree map rectangles proportion hierarchy area" },
              ...
              { type: "waffle",    label: "Waffle",    description: "Shares as a 10×10 grid of squares — the honest pie. Takes a 2-column frame (label, value) for category shares, or a single value between 0 and 1 for a plain fraction of the grid.", create: () => new WaffleNode(), parity: false, keywords: "waffle dot matrix squares proportion percentage share progress pictogram" },
            ],
          },
          {
          ...
          },
          {
            ...
            children: [
              ...
              { type: "calendar-heatmap", label: "Calendar", description: "A year of daily activity as a weeks-by-weekdays grid, each day tinted by its value — the contribution-graph look. Takes a 2-column frame (Date, Value); duplicate days sum.", create: () => new CalendarHeatmapNode(), parity: false, keywords: "calendar heatmap daily activity year contribution github days streak" },
            ],
          },
        ],
      },
      ...
      { type: "pair", children: [
        { type: "note", label: "Note", description: "A free-floating markdown note with a title and body, in any position and any palette tint. Open the body with a ----fenced YAML block to turn each key into a typed OUTPUT socket, so a note doubles as a constants source. A Note only emits; to read live graph values into a document, use a Report.", create: () => new NoteNode(), parity: false },
        { type: "report", label: "Report", description: "A standalone markdown document, separate from the graph: prose with inline `=name` refs that show a wired value formatted in the text, plus Notes and charts embedded as placed objects. Opens from the small anchor card.", create: () => new ReportNode(), parity: false },
      ]},
  ...
  // ── LOGIC ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "comparison", label: "Comparison",  description: "Compare two values → 1 for true or 0 for false.", create: () => new ComparisonNode() },
      {
