...
import {
  ...
  SparklineNode, ChartNode, GaugeNode, HeatmapCellNode, ChartBuilderNode,
...
const arithLeaf    = (op: ArithmeticOp):   NodeCatalogEntry => ({ type: `arith-${op}`,     label: ARITHMETIC_OP_META[op].label,     description: ARITHMETIC_OP_META[op].description,     create: () => new ArithmeticNode({ op })    });
...
export const NODE_CATALOG: CatalogEntry[] = [
  ...
  // ── OUTPUT ───────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "note", label: "Note", description: "A free-floating markdown note — title + body, drag it anywhere, tint it. Open the body with a ----fenced YAML block to turn each key into a typed OUTPUT socket (a note as a constants source). An inline `=name` code span mints a wireable INPUT that shows the connected value in the prose.", create: () => new NoteNode(), parity: false },
  ...
  // ── TABLES & FRAMES ───────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          { type: "distinct",    label: "Distinct",    description: "Remove duplicate rows from a Frame, keeping the first of each (the table form of UNIQUE).", create: () => new DistinctNode(), parity: false },
          ...
          { type: "filter-frame",label: "Filter Rows", description: "Keep a Frame's rows whose chosen column passes a test (>, =, contains…). Chain for AND. Blanks/errors drop (Excel: =FILTER).", create: () => new FilterFrameNode(), parity: false },
          { type: "join",        label: "Join",        description: "Combine two Frames on a key column — inner / left / right / outer, or as-of (nearest match on a sorted number/date key, with a direction + tolerance). A left row matching several right rows fans out (as-of never fans out). The relational JOIN (Excel: VLOOKUP/XLOOKUP do a left join one column at a time).", create: () => new JoinNode(), parity: false },
          { type: "frame-lookup",label: "Frame Lookup", description: "Look up a value in one column of a Frame and return the matching cell from another column — VLOOKUP / XLOOKUP for tables. Exact match by default; an approximate mode (≤/≥) falls back to the closest smaller/larger number or date. If-not-found falls back, else #N/A (Excel: =XLOOKUP / =VLOOKUP).", create: () => new FrameLookupNode(), parity: false, keywords: "xlookup vlookup lookup frame table match find" },
          ...
          { type: "group-by-frame", label: "Group By", description: "Group a Frame's rows by one or more key columns and aggregate one column (sum / average / min / max / count) — a PivotTable's row grouping (Excel: =GROUPBY).", create: () => new GroupByFrameNode(), parity: false },
