...
export const NODE_CATALOG: CatalogEntry[] = [
  // ── INPUT ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "cube-input", label: "Cube Input", description: "Type a Cube directly: rows of records where a value can be a number, text, a list, or a nested table. Open a cell in the popup to edit its list or nested table in place.", accent: NODE_KIND_ACCENTS.frame, create: () => new CubeInputNode(), parity: false, keywords: "cube input literal type records nested list json source" },
      { type: "pair", children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "data-feed",     label: "Data Feed",   description: "Live economic and market data as a Frame: FRED series with no key, stock history via Alpha Vantage with a free key. Stores the id or ticker, not the data. Desktop takes any URL.", create: () => new DataFeedNode(), parity: false },
    ...
    children: [
      ...
      { type: "format-controller", label: "Format Controller", description: "Sets a docked socket's number format (decimal, fraction, %, currency…) and a unit label like `°C`, `m`, or `kg`. Units must match on connected cables.", create: () => new FormatControllerNode() },
      {
        ...
        children: [
          ...
          { type: "record",    label: "Record",    description: "One frame row as labeled boxes, or every row as a gallery of cards, a board of lanes grouped by a column, or an indented list. A cell holding an image URL shows the picture.", create: () => new RecordNode(), parity: false, keywords: "record card form detail row browse fields layout boxes airtable gallery kanban board lanes list outline title size clamp" },
          { type: "gantt",     label: "Gantt",     description: "Draws a scheduled project: a bar per task, milestones, summary brackets, dependency arrows, the critical path, a baseline and a status line. Reads the rows Schedule worked out.", create: () => new GanttNode(), parity: false, keywords: "gantt chart project schedule timeline plan tasks bars milestones critical path dependencies predecessors baseline waterfall roadmap pert cpm" },
      ...
      { type: "pair", children: [
        ...
        { type: "query", label: "Query", description: "A Composite pre-shaped for data transformation: table in, verb chain inside, result out. Upstream changes only mark it stale until you press Refresh. Excel: Power Query.", create: () => new CompositeNode({
          label: "Query",
          runMode: "manual",
      ...
      { type: "alert",     label: "Alert",    description: "Watch a value and fire a toast plus an Alerts HUD entry on a status change. Modes: range with Low/High thresholds, boolean where `TRUE` fires, change on any new value, or threshold-cross.", create: () => new AlertNode() },
      {
        ...
        children: [
          ...
          { type: "tornado", label: "Tornado", description: "One-at-a-time sensitivity ranking: Run perturbs each upstream Number or Slider ±10% (or its min/max) and ranks the inputs by how far this value swings, in an inline tornado chart.", create: () => new TornadoNode(), parity: false, keywords: "tornado sensitivity analysis what-if one at a time impact ranking swing trust" },
        ],
      },
  ...
  // ── NUMBERS ──────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      { type: "pair", children: [
        ...
        { type: "equation", label: "Equation", description: "A relation like `V = I * R` where every variable is both input and output. Leave one empty and it solves. A quadratic returns every real root. All given: Check turns `TRUE` or `FALSE`.", create: () => new EquationNode(), accent: NODE_KIND_ACCENTS.math, keywords: "solve rearrange unknown goal seek formula bidirectional check quadratic roots" },
      ]},
      ...
      },
      {
        ...
        children: [
          { type: "distribution", label: "Distribution", description: "Every probability distribution (normal, t, chi-squared, F, binomial, Poisson…) as CDF, PDF or PMF, a tail, or the inverse. Excel: the `NORM.DIST` / `T.INV` / `BINOM.DIST` families.", create: () => new DistributionNode(), keywords: "distribution probability cdf pdf pmf inverse quantile percentile critical value tail gaussian bell curve critbinom phi gauss standard normal density" },
          { type: "pair", children: [
    ...
    children: [
      ...
      ]},
      {
        ...
        children: [
          ...
          { type: "import-obsidian", label: "Import Obsidian Note", description: "Picks a `.md` note from your Obsidian vault as a read-only Note: frontmatter becomes typed outputs, the body renders inline. Reload re-reads from disk. Desktop only.", create: () => new ImportObsidianNode(), parity: false, keywords: "obsidian vault markdown md note import read source frontmatter" },
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "pair", children: [
            ...
            { type: "list-index",  label: "INDEX",   description: "Reads a cell from any container: the nth of a list, (Row, Column) of a Matrix, Frame or Cube. An empty Row or Column selects the whole column or row. Excel: `INDEX`.", create: () => new ListIndexNode(), keywords: "cube frame cell nested drill get cell unnest slice whole row column" },
          ]},
      ...
      },
      {
        ...
        children: [
          ...
          { type: "ets-forecast", label: "Forecast (ETS)", description: "Holt–Winters exponential smoothing: a forecast N steps ahead with a growing 95% band, season length detected or set. statsmodels `ExponentialSmoothing`, R `HoltWinters`. Excel: `FORECAST.ETS`, close but not identical.", create: () => new EtsForecastNode(), parity: false, keywords: "forecast ets exponential smoothing holt winters seasonal time series predict trend season confint seasonality" },
    ...
    children: [
      {
        ...
        children: [
          ...
          { type: "returns", label: "Returns", description: "The return-series one-liners: log or simple returns, cumulative return, drawdown and max drawdown, CAGR, annualized volatility, Sharpe and Sortino. pandas `pct_change`.", create: () => new ReturnsNode(), parity: false, keywords: "log returns returns log return pct_change cumulative drawdown max drawdown cagr volatility sharpe sortino risk-free annualize annualise quant performance portfolio price series" },
      ...
      },
      {
        ...
        children: [
          { type: "bond-pricing", label: "Bond Pricing", description: "A coupon bond's clean price from its yield, or yield from price, on a `30/360` basis, with odd first or last coupon forms. Excel: `PRICE`, `YIELD` and the `ODDF` / `ODDL` forms.", create: () => new BondPricingNode(), parity: false, keywords: "bond price yield coupon clean price yield to maturity ytm odd first last irregular period redemption par frequency" },
        ],
      },
      {
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "date-trunc", label: "Truncate Date", description: "Floors a date to the start of its day, week, month, quarter or year, or ceilings to the next: the period bucket for a Group By. lubridate `floor_date`, pandas `to_period`, SQL `DATE_TRUNC`. Excel: `DATE(YEAR(d), MONTH(d), 1)`.", create: () => new DateTruncNode(), parity: false, keywords: "truncate floor ceiling date period month week quarter year resample bucket floor_date date_trunc to_period start of month" },
        ],
      },
      { type: "pair", children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "pair", children: [
            ...
            { type: "fuzzy-match",     label: "Fuzzy Match",     description: "The closest candidate to a text, and its score, above a threshold: typo-tolerant matching of names or SKUs. rapidfuzz `process.extractOne`, R stringdist `amatch`, Excel's Fuzzy Lookup add-in.", create: () => new FuzzyMatchNode(), parity: false, keywords: "fuzzy match lookup approximate nearest string typo dedupe reconcile names extractOne amatch" },
          ]},
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "sumifs", label: "SUMIFS", description: "Aggregates one frame column (sum, count, average, min, max) under criteria rows on the others, matching all or any of them. Excel: `SUMIFS`, `COUNTIFS`, `AVERAGEIFS`, `MINIFS`, `MAXIFS`.", create: () => new SumIfsNode(), keywords: "sumif countif averageif minif maxif criteria conditional aggregate frame all any or" },
          ...
          { type: "group-by-frame", label: "GROUPBY", description: "Groups rows by key columns and aggregates one column (sum, average, min, max, count), with optional grand-total and subtotal rows. Keys group case-sensitively. Excel: `GROUPBY`.", create: () => new GroupByFrameNode(), parity: false },
          { type: "pair", children: [
            ...
            { type: "bind-columns", label: "Bind Columns", description: "Frames side by side by position: a repeated name gets a suffix, a shorter Frame pads with blanks. pandas `concat(axis = 1)`, R `bind_cols`. Excel's `HSTACK`, for frames.", create: () => new BindColumnsNode(), parity: false, keywords: "bind_cols cbind concat axis 1 side by side zip frames columns hstack horizontal" },
          ]},
          {
            ...
            children: [
              { type: "fill-blanks", label: "Fill Down", description: "Fills blank cells from the neighboring row: Down carries the last value forward, Up the next one back. Name columns or leave blank for all. Power Query: Fill Down / Fill Up.", create: () => new FillBlanksNode(), parity: false, keywords: "fill down up forward backward blanks nulls merged cells carry propagate ffill bfill clean" },
          ...
          },
          {
            ...
            children: [
              { type: "pivot",       label: "PIVOTBY",     description: "Cross-tab long → wide: Row and Column fields with multi-level headers, per-value aggregates (`SUM`, `AVERAGE`, `COUNT`…), grand totals and subtotals, sort, filter and % running totals.", create: () => new PivotNode(), parity: true },
          ...
          },
          {
            ...
            children: [
              ...
              { type: "corr-matrix", label: "Correlation Matrix", description: "Pairwise correlation (Pearson, Spearman or Kendall) or covariance between every pair of number columns, as a frame with a leading name column. pandas `df.corr`, R `cor`.", create: () => new CorrMatrixNode(), parity: false, keywords: "correlation matrix corr cov covariance pearson spearman kendall pairwise heatmap" },
              { type: "reconcile",   label: "Reconcile",   description: "Compares two versions of a Frame by key: each row Added, Removed, Changed or Unchanged, with before, after and Δ per numeric column, and an optional Price / Volume / Mix variance.", create: () => new ReconcileNode(), parity: false, keywords: "reconcile compare diff variance price volume mix pvm audit changed added removed data quality trust" },
            ],
          },
          {
            ...
            children: [
              ...
              { type: "earned-value", label: "Earned Value", description: "Measures a project against plan and budget at a status date: planned and earned value, actual cost, schedule and cost variances, SPI, CPI and EAC. Cost keeps its currency.", create: () => new EarnedValueNode(), parity: false, keywords: "earned value evm bcws bcwp acwp planned earned actual cost variance schedule variance spi cpi eac vac tcpi budget baseline project performance" },
              ...
              { type: "settle",      label: "Group Cost Settle", description: "Squares up a group that paid unevenly: who pays whom in the fewest transfers, plus each person's Paid, Owes, Owed and Net, from per-person totals or an expense ledger.", create: () => new SettleNode(), parity: false, keywords: "settle settlement split bill trip expenses transactions ledger who owes whom debts transfers splitwise group cost share even up payer participants" },
            ],
          },
        ],
      },
      {
        ...
        children: [
          { type: "nest-join", label: "Nest Join", description: "Nests two Frames into a Cube on a shared key: each Parent row gains a cell of its matching Child rows, so nothing fans out. Repeat to go a level deeper. tidyr `nest_join`.", create: () => new NestJoinNode(), parity: false, keywords: "cube nest join relate nest_join relational merge group hierarchy multi-level deepen" },
          ...
          { type: "cube-rollup", label: "Cube Rollup", description: "Aggregates a column inside each row's nested sub-table and flattens the Cube back to a Frame with the roll-up appended: an assembly's cost as the `SUM` of its parts. Same ops as Group By.", create: () => new CubeRollupNode(), parity: false, keywords: "cube rollup aggregate sum bom bill of materials costing nested cost roll up assembly subtotal" },
        ],
      },
      {
      ...
      },
      {
        ...
        children: [
          ...
          { type: "reduce-lambda", label: "REDUCE", description: "Folds a list or table to one value from Initial. Variables `acc` (running accumulator), `value` (this element), `step` (1-based position). Pick the result type for text or date. Excel: `REDUCE`.", create: () => new ReduceLambdaNode(), parity: false },
          { type: "scan-lambda", label: "SCAN", description: "`REDUCE` that keeps every running value: emits the accumulator after each cell, in the input's shape. A running total is `acc + value` from `0`. Variables `acc`, `value`, `step`. Excel: `SCAN`.", create: () => new ScanLambdaNode(), parity: false, keywords: "running total cumulative accumulate prefix sum" },
        ],
      },
      {
