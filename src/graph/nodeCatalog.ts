import {
  ...
  ColumnsNode, GroupByFrameNode, PivotNode, UnpivotNode, NestNode, UnnestNode, AppendNode, BindColumnsNode, RenameNode, SplitColumnNode, AddIndexNode, DecisionMatrixNode, DecisionSensitivityNode, BudgetAllocatorNode,
  ReconcileNode,
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { type: "budget-allocator", label: "Budget Allocator", description: "Splits money across categories that each have a min and max price and a value weight. Fit budget spends the whole budget in proportion to the weights, kept inside every range, and the slack a capped category leaves flows to the rest. Min for target finds the least spend that reaches a weighted-value goal, buying the most-valued categories first. Min proportional finds the least spend that keeps every category in proportion to its weight above its floor. Water-filling, no solver. Output: Category and Allocation.", create: () => new BudgetAllocatorNode(), parity: false, keywords: "budget allocator allocate allocation split spend divide categories weights value water-filling waterfilling proportional minimize target knapsack money planner portfolio" },
