import {
  ...
  TrendNode, EtsForecastNode, DecomposeNode, OdeIntegrateNode, FitDistributionNode, InterpolateNode, LinestNode, LogestNode, BinomDistRangeNode,
  NODE_KIND_ACCENTS,
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "list-peaks", label: "Find Peaks", description: "Positions and heights of the local maxima, filtered by minimum height, spacing and prominence. scipy.signal.find_peaks, R pracma::findpeaks.", create: () => new FindPeaksNode(), parity: false, keywords: "peaks find_peaks local maxima maximum prominence spikes signal detect" },
          { type: "pair", children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "decompose", label: "Decompose", description: "Splits a seasonal series into trend (centered moving average), a repeating seasonal pattern and the residual — additive or multiplicative. statsmodels seasonal_decompose, R decompose.", create: () => new DecomposeNode(), parity: false, keywords: "decompose decomposition seasonal trend residual seasonality time series stl classical moving average deseasonalize" },
          { type: "ode-integrate", label: "ODE Integrate", description: "Integrates dy/dt = f(t, y) with classic fixed-step RK4, from t0 to t1 in a set number of steps. Type the derivative as an expression in t and y. Outputs the t and y sample points. scipy solve_ivp, R deSolve.", create: () => new OdeIntegrateNode(), parity: false, keywords: "ode integrate rk4 solve_ivp deSolve differential equation euler runge kutta initial value" },
    ...
    children: [
      ...
      { type: "date-time",     label: "TIME",      description: "Builds a time fraction 0–1 from Hour, Minute, Second. Add it to a date serial for date+time. Excel: TIME.", create: () => new TimeConstructNode(), parity: false },
      {
        ...
        children: [
          ...
          { type: "pair", children: [
            { type: "date-epoch-from", label: "Epoch → Date", description: "Unix time (seconds or milliseconds since 1970-01-01 UTC) → a date. pandas to_datetime(unit=), R as.POSIXct(origin=). Excel: n/86400 + 25569.", create: () => new EpochNode({ op: "from" }), parity: false, keywords: "epoch unix timestamp posix seconds milliseconds 1970 utc to_datetime" },
