...
export const NODE_CATALOG: CatalogEntry[] = [
  ...
  // ── NUMBERS ──────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "script", label: "Script", description: "A JavaScript function like (a, b) => a * b: its parameters become inputs and its return value is the result. Inputs arrive as numbers, text, booleans, dates as serials, and lists or lists of rows; a blank is null. The result is folded onto the declared type, so a value of the wrong kind is #TYPE! and an unusable shape is #SHAPE!. Runs sandboxed with no network or files, and stops after one second.", keywords: "script javascript js code function program custom", create: () => new ScriptNode(), accent: NODE_KIND_ACCENTS.math },
      {
