...
export const NODE_CATALOG: CatalogEntry[] = [
  // ── INPUT ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "lambda-make",   label: "LAMBDA",      description: "Defines a reusable formula as a value. Parameters go in the λ(…) row; other variables become captured inputs. Parameters bind positionally. Like Expression, the formula evaluates via Formula.js with standard Excel functions, separate from the visual nodes. Excel: LAMBDA.", accent: NODE_KIND_ACCENTS.lambda, create: () => new LambdaNode(), parity: false },
  ...
  // ── NUMBERS ──────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      { type: "expression", label: "Expression", description: "A formula like a*b+1: named variables become input sockets. Math functions, the constants pi / tau / e / phi, element-wise broadcasting over lists AND matrices, the dynamic-array core (TRANSPOSE, MMULT, SEQUENCE…), complex numbers, and LAMBDA as a value. The result type loops any function over arrays (e.g. UPPER(name), DATE(y,m,d)). Functions resolve through the same registry as the visual nodes, so a name here computes what its node computes. Frames and cubes stay out by design — the table verbs are nodes, and a Computed Column carries row formulas.", create: () => new ExpressionNode(), accent: NODE_KIND_ACCENTS.math },
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "computed-column", label: "Computed Column", description: "Adds a column computed row by row: @name reads this row's cell, a bare column name is the whole column — so @revenue / SUM(revenue) is a share of total. [Unit Price] and @[Unit Price] spell the two for names a variable can't; a λ input carries one reusable formula across frames   (Power Query: Custom Column)", keywords: "custom column calculated field formula derive mutate row-wise index this-row @", create: () => new ComputedColumnNode(), parity: false },
        ],
      },
      {
      ...
      },
      {
        ...
        children: [
          { type: "nest-join", label: "Nest Join", description: "Nests two Frames into a Cube on a shared key column: each Parent row gains a cell holding the sub-Frame of matching Child rows. A flat Join fans out into repeated rows; this keeps one row per parent and tucks the children inside. A previous Nest Join's Cube fed back in as the Parent deepens the hierarchy one level (Customer → Order → LineItem). Equivalent: tidyr nest_join, Power Query merge-without-expand.", create: () => new NestJoinNode(), parity: false, keywords: "cube nest join relate nest_join relational merge group hierarchy multi-level deepen" },
