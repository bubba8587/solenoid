import {
  ...
  TrendNode, EtsForecastNode, DecomposeNode, FitDistributionNode, InterpolateNode, LinestNode, LogestNode, BinomDistRangeNode,
  NODE_KIND_ACCENTS,
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { type: "cx-polyroots", label: "Polynomial Roots", description: "Every root of a polynomial from its coefficient list (highest degree first), complex pairs included, with the real roots alone as a second output. numpy.roots, R polyroot. No Excel equivalent.", create: () => new PolyRootsNode(), parity: false, keywords: "polynomial roots polyroot np.roots zeros solve cubic quartic complex factor" },
            ],
          },
        ],
      },
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "decompose", label: "Decompose", description: "Splits a seasonal series into trend (centered moving average), a repeating seasonal pattern and the residual — additive or multiplicative. statsmodels seasonal_decompose, R decompose. No Excel equivalent.", create: () => new DecomposeNode(), parity: false, keywords: "decompose decomposition seasonal trend residual seasonality time series stl classical moving average deseasonalize" },
    ...
    children: [
      {
        ...
        children: [
          ...
          { type: "returns", label: "Returns", description: "The return-series one-liners: log / simple returns, cumulative return, drawdown and max drawdown, CAGR, annualized volatility, Sharpe and Sortino — pick on the card. pandas pct_change / cumprod, PerformanceAnalytics, quantmod. No Excel equivalent beyond hand-built columns.", create: () => new ReturnsNode(), parity: false, keywords: "returns log return pct_change cumulative drawdown max drawdown cagr volatility sharpe sortino risk-free annualize annualise quant performance portfolio price series" },
    ...
    children: [
      ...
      { type: "pair", children: [romanArabicLeaf("roman"), romanArabicLeaf("arabic")] },
      {
        ...
        children: [
          ...
          { type: "pair", children: [
            { type: "text-pad", label: "Pad Text", description: "Pads text to a width with a fill character — on the left, right, or both sides. Python ljust / rjust / center, R str_pad. No Excel equivalent (REPT arithmetic).", create: () => new PadTextNode(), parity: false, keywords: "pad padding ljust rjust center justify align width fill zero-pad str_pad fixed width column" },
            { type: "text-truncate", label: "Truncate Text", description: "Cuts text to a maximum width, ending in an ellipsis when anything was cut. R str_trunc, textwrap.shorten. No Excel equivalent.", create: () => new TruncateTextNode(), parity: false, keywords: "truncate shorten ellipsis clip cut width max length abbreviate str_trunc" },
          ]},
    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "by-axis",    label: "BYROW / BYCOL", description: "Reduces each row or column of a table to one value. Variable v = the row or column as a list. Pick the result type for text or date. Excel: BYROW or BYCOL.", create: () => new ByAxisNode(), parity: false },
        ...
        children: [
          ...
          },
          {
            ...
            children: [
              ...
              { type: "kmeans", label: "K-Means", description: "Groups the rows into k clusters by their number columns (k-means++ seeding, the best of ten runs): a cluster id per row and a centers frame. sklearn KMeans, R kmeans. No Excel equivalent.", create: () => new KMeansNode(), parity: false, keywords: "kmeans k-means cluster clustering segment centroid unsupervised group rows" },
              { type: "pca", label: "PCA", description: "Principal components of the number columns: the rows in the new axes (scores), how each feature loads on each axis, and the share of variance each axis explains — centered, or standardized first. sklearn PCA, R prcomp. No Excel equivalent.", create: () => new PcaNode(), parity: false, keywords: "pca principal component analysis dimensionality reduction loadings scores explained variance prcomp eigen" },
              { type: "logistic", label: "Logistic Regression", description: "Fits a 0/1 target on the other number columns: log-odds coefficients with standard errors, z and p, plus the fitted probability per row. Unregularized maximum likelihood (IRLS). R glm(binomial), statsmodels Logit. No Excel equivalent.", create: () => new LogisticNode(), parity: false, keywords: "logistic regression logit glm binomial classification probability odds ratio irls sigmoid" },
