    ...
    children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "local-file",   label: "Local File",  description: "Loads a Frame from your data folder (Settings ▸ Data). A `.parquet` reads straight into the native engine, so typed columns arrive intact with no inference. A Project XML file, or a CSV whose Predecessors use row numbers (3FS+2d), also comes out as a Plan: tasks nested as the outline with their predecessors resolved to names, ready for Schedule. Anything else is read as CSV with columns auto-typed. Stores the file name. Refresh to re-read. Desktop only.", create: () => new LocalFileNode(), parity: false, keywords: "csv parquet arrow column columnar native engine polars file load import project xml mspdi plan smartsheet" },
          { type: "pair", children: [
