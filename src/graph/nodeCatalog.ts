...
export const NODE_CATALOG: CatalogEntry[] = [
  // ── INPUT ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "save-times",    label: "Save Times",  description: "When this document was last autosaved and when it was last written to a file, as two date values. Both are session-scoped, so they read blank until the first save of a session.", create: () => new SaveTimesNode(), parity: false, keywords: "save autosave saved timestamp version document file written when last clock" },
      { type: "pair", children: [
      ...
      },
      {
        ...
        children: [
          ...
          { type: "pair", children: [
            ...
            { type: "list-index",  label: "INDEX",   description: "Reads a cell out of any container: the nth of a list, (Row, Column) of a Matrix, the cell of a Frame / Cube. Leaving Row or Column empty (or 0) selects the WHOLE column / row. A nested Frame/Cube cell comes out whole — how data comes back out of a Cube. Excel: INDEX(range, row, col).", create: () => new ListIndexNode(), keywords: "cube frame cell nested drill get cell unnest slice whole row column" },
          ]},
  ...
  // ── LOGIC ────────────────────────────────────────────────────────────────────
  {
    ...
    children: [
      ...
      { type: "iferror", label: "IFERROR / IFNA", description: "Returns Fallback when Value is an error (IFNA: only #N/A). A blank is not an error and passes through. Excel: IFERROR / IFNA.", create: () => new IFErrorNode() },
