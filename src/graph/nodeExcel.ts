...
export const NODE_EXCEL: Record<string, ExcelEquiv[]> = {
  ...
  "list-cumulative": [{ excel: "SCAN", syntax: "=SCAN(init, array, LAMBDA(acc, x, ...))", parity: false, note: "Solenoid's Cumulative covers CumSum/CumMax/…; for an arbitrary formula, REDUCE gives the final value — a formula SCAN (running intermediates) isn't built" }],
  "list-drop": [{ excel: "DROP", syntax: "=DROP(list, count)", parity: false, note: "The 1-D list spelling; DROP (table) is the 2-D rows+cols one" }],
  "list-filter": [{ excel: "FILTER", syntax: "=FILTER(array, include)", parity: false, note: "Conditions live on the card (extensible AND/OR rows) instead of an include vector; filtering by a PARALLEL list is the SUMIFS node or Frame from Lists → Filter Rows" }],
