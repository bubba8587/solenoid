...
export const SCIENTIFIC_PACK: Pack = {
  ...
  group: "Analysis",
  nodes: [
    { path: PATH, entry: { type: "list-spectrum", label: "Spectrum (FFT)", description: "Frequency content of a signal: one row per bin with frequency (in the sample rate's units), magnitude (a sine of amplitude A reads A) and phase. Any length. `numpy.fft.rfft`, R `fft`, MATLAB `fft`.", create: () => new SpectrumNode(), parity: false, keywords: "fft fourier spectrum frequency dft periodogram harmonics signal vibration rfft" } },
    ...
    { path: PATH, entry: { type: "ode-integrate", label: "ODE Integrate", description: "Integrates `dy/dt = f(t, y)` by fixed-step RK4 from `t0` to `t1` in a set number of steps, the derivative given as a lambda of t and y. Outputs `t` and `y` columns. scipy `solve_ivp`, R `deSolve`.", create: () => new OdeIntegrateNode(), parity: false, keywords: "ode integrate rk4 solve_ivp deSolve differential equation euler runge kutta initial value lambda" } },
