...
import { cubeIn, strIn, chartOut, readInput } from "./shared";
...
// columns into the figure's data payload. The card carries the [Chart] chip; the
// figure draws in the Display, the popup and a Report (the Record precedent).

/** Today as the LOCAL calendar day, the reference for the drawn today line. */
...
export class GanttNode extends ClassicPreset.Node {
  ...
  data(inputs: {
    ...
    const payload = ganttPayloadFromSchedule(schedule, { baseline, today: todaySerial(), options: optStr ?? "" });
