...
**A palette may replace this whole neutral ramp** — canvas, window, the three surfaces, the three borders, the four inks, per theme mode (`BUILTIN_CHROME` in `palette.ts`). The rest of the neutral chrome (panels, overlays, button hover, the gauge track, the selected cable, light-theme shadows) DERIVES from those thirteen by fixed mixes, so a palette moves one ramp rather than forty tokens; the mix steps are calibrated against App.css's own literals, not eyeballed per palette. Authoring a ramp is opt-in and rare (D35) — recoloring the graph is what a palette does, recoloring the workbench is a separate claim, and one palette makes it: **Orchard**, lifted from the Pear design system, which trades the near-black workbench for warm cream and warm ink. A palette that authors nothing keeps the hexes above, and `appTheme` CLEARS the vars rather than leaving the last palette's chrome behind. Note what this does NOT relax: a palette moves the neutrals as a SET, keeping every tonal relationship in this section (canvas darker than card, field the brightest layer in light, four distinct ink tiers). It is not a license to tint one token.
